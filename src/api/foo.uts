import { API_DOMAINS, http } from '../http/request';
import type { LimeRequestConfig } from '@/uni_modules/lime-request';
import { DEFAULT_OSS_UPLOAD_PATH, uploadFile } from '../utils/upload.uts';
import type { UploadFileOptions } from '../utils/upload.uts';

export const FOO_UPLOAD_PATH: string = DEFAULT_OSS_UPLOAD_PATH;

// ==========================================
// 类型定义
// ==========================================

export type IFoo = {
  id: any | null;
  name: string;
};

export type IFooList = {
  list: IFoo[];
  total: number;
  page: number;
  pageSize: number;
};

// ==========================================
// Mock 数据
// ==========================================

const MOCK_FOO_LIST: IFoo[] = [
  { id: 1, name: 'unix' },
  { id: 2, name: 'UnibestX' },
  { id: 3, name: 'lime-request' }
];

// ==========================================
// API 函数
// ==========================================

/**
 * 获取 Foo 列表（使用 mock 数据）
 */
export function getFooList(_params: UTSJSONObject | null = null): Promise<IFoo[]> {
  return Promise.resolve(MOCK_FOO_LIST);
}

/**
 * 获取单个 Foo（使用 mock 数据）
 */
export function getFooById(id: number): Promise<IFoo | null> {
  const found = MOCK_FOO_LIST.find((item: IFoo): boolean => item.id === id);
  return Promise.resolve(found ?? null);
}

/**
 * GET 请求示例 —— 对应 foo-alova.ts 中的 foo() 函数
 * 使用次要域名（SECONDARY），参考 API_DOMAINS 配置
 */
export function foo(): Promise<IFoo> {
  return http.get<UTSJSONObject>('/foo', {
    params: {
      name: 'unix',
      page: 1,
      pageSize: 10
    } as UTSJSONObject,
    baseURL: API_DOMAINS.SECONDARY, // 切换到次要域名
    extra: {
      ignoreAuth: true // 此接口不需要鉴权
    } as UTSJSONObject
  } as LimeRequestConfig).then((data: UTSJSONObject): IFoo => {
    return {
      id: (data.get('id') ?? '') as any,
      name: data.getString('name') ?? ''
    } as IFoo;
  });
}

/**
 * POST 请求示例 —— 新建一个 Foo
 */
export function createFoo(data: IFoo): Promise<IFoo> {
  return http.post<UTSJSONObject>('/foo', data).then((res: UTSJSONObject): IFoo => {
    return {
      id: (res.get('id') ?? '') as any,
      name: res.getString('name') ?? ''
    } as IFoo;
  });
}

/**
 * PUT 请求示例 —— 更新一个 Foo
 */
export function updateFoo(id: number, data: IFoo): Promise<IFoo> {
  return http.put<UTSJSONObject>(`/foo/${id}`, data).then((res: UTSJSONObject): IFoo => {
    return {
      id: (res.get('id') ?? '') as any,
      name: res.getString('name') ?? ''
    } as IFoo;
  });
}

/**
 * DELETE 请求示例 —— 删除一个 Foo
 */
export function deleteFoo(id: number): Promise<any> {
  return http.delete<any>(`/foo/${id}`);
}

/**
 * 上传文件示例 —— 使用 utils/upload.uts
 * 上传成功返回 OSS 地址
 * @param filePath 本地文件路径
 * @param ignoreAuth 是否跳过鉴权（默认 true 用于演示接口）
 * @param uploadPathOrUrl 动态上传接口路径或完整 URL（不传默认使用 FOO_UPLOAD_PATH）
 */
export function uploadFooFile(
  filePath: string,
  ignoreAuth: boolean = false,
  uploadPathOrUrl: string = FOO_UPLOAD_PATH
): Promise<string> {
  return uploadFile({
    url: uploadPathOrUrl,
    filePath,
    formData: null,
    ignoreAuth
  } as UploadFileOptions);
}
