...
export const props = defineMixin({
  props: {
    ...
    // 是否允许下滑关闭（仅在 mode == 'bottom' 时生效）
    closeOnSlideDown: {
      type: Boolean,
      default: () => defProps.popup.closeOnSlideDown
    },
    // 下滑关闭阈值 (px)
    slideDownThreshold: {
      type: [Number, String],
      default: () => defProps.popup.slideDownThreshold
    },
    // 弹窗内部滚动容器 ID
    scrollId: {
      type: String,
      default: () => defProps.popup.scrollId
    },
    // 是否显示顶部拖拽手柄条
    showDragBar: {
      type: Boolean,
      default: () => defProps.popup.showDragBar
    },
    dragBarClass: {
      type: String,
      default: () => defProps.popup.dragBarClass
    },
    dragWrapClass: {
      type: String,
      default: () => defProps.popup.dragWrapClass
    }
  }
});
