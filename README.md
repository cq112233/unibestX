...
> unibestX 项目通过 **HBuilderX** 运行，不使用命令行方式。
...
- **H5 平台**：在 HBuilderX 中选择 `运行 → 运行到浏览器`，自动打开 `http://localhost:9001/`。
- **微信小程序**：在 HBuilderX 中选择 `运行 → 运行到小程序模拟器 → 微信开发者工具`，自动编译到 `dist/dev/mp-weixin`。
...
本项目参考自 [unibest](https://github.com/unibest-tech/unibest)，官网地址：[https://unibest.tech/](https://unibest.tech/)
