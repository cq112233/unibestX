...
**架构：**
- `src/store/official/`：存放基于官方 Pinia 与 `pinia-plugin-persistedstate` 的组合式实现（`index.ts`, `app.ts`, `user.ts`, `token.ts`）；
...
- **创建：** `src/store/official/index.ts`、`app.ts`、`user.ts`、`token.ts` — 封装官方 Pinia (Vapor / Web / iOS / Harmony / 小程序) 模式实现。
...
- [ ] **步骤 1：创建 `src/store/vdom/` 目录并将现有代码移入**
...
- [ ] **步骤 4：创建 `src/store/official/token.ts`**
  ...
  - 开启 `persist: { key: 'pinia:token' }`

---

### 任务 3：构建门面层条件编译分发 (`src/store/*.uts`)

- [ ] **步骤 1：重构 `src/store/index.uts`**
  - `#ifdef APP-ANDROID && !VUE3-VAPOR` 导入 `vdom`
  - `#ifndef APP-ANDROID && !VUE3-VAPOR` 导入 `official`
- [ ] **步骤 2：重构 `src/store/app.uts`、`user.uts`、`token.uts`**
...
- [ ] **步骤 1：运行 `npm run lint`**
  - 确保 0 错误
- [ ] **步骤 2：校验规范文档更新与提交 Git**
