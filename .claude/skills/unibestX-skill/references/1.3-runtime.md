...
- **`||` 表达式是 `new Function('with(context){ return (A || B) }')` 求值的**（`preprocess.js` 的 `testPasses`，实测）：操作数里只要有一个名字**不在 context 上**就抛 `ReferenceError`，而异常被 `testPasses` 静默吞成 `false` —— **整段无声消失，编译不报错**。内置平台宏踩不到（`DEFAULT_KEYS` 把 `H5` / `WEB` / `MP` / `VUE3_VAPOR` 等**全量预置为 `false`**，未选中的平台是「已定义且为 false」而非缺失）；**自定义宏会踩**：`#ifdef MY_FLAG || H5` 在所有没定义 `MY_FLAG` 的端都会被整段剔除。
...
- **验证手段（不要靠肉眼看 `#ifdef`）**：直接用框架自带的预处理器对源文件求值，一次跑满全部目标平台上下文。本项目脚本 `/tmp/chg/check-pre.cjs`、`/tmp/chg/check-iconfont-pre.cjs`：

  ```js
  const { preprocess } = require('<项目>/node_modules/@dcloudio/uni-cli-shared/lib/preprocess/lib/preprocess.js');
  const { initScopedPreContext } = require('<项目>/node_modules/@dcloudio/uni-cli-shared/dist/preprocess/context.js');
  process.env.UNI_APP_X_DOM2 = 'true';   // VUE3_VAPOR 只由这个 env 决定（不是 manifest）
  const ctx = initScopedPreContext('h5', undefined, undefined, true).preUVueContext;
  const out = preprocess(src, ctx, { type: 'js' });
  ```

  - **`ctx` 必须走 `initScopedPreContext()`，严禁手搓**：手搓 `{ WEB: true }` 会缺掉「已定义但为 false」的 `H5`，`#ifdef H5 || WEB` 就抛 `ReferenceError` 被吞成 `false`，凭空造出「web 端分支丢了」这类**假失败**（本项目已踩过）。本项目 `uni-launch web` 走的是 `platform === 'web'`，该分支下 `WEB = true` 而 `H5 = false` —— 所以 `#ifdef H5` 会**漏掉整个 web 构建**，必须写 `H5 || WEB`。
  - `.uvue` 要**按 SFC 分块**跑（template 用 `html` 规则、script / style 用 `js` 规则，`preCss === preJs`），整份文件一把梭会漏掉分块语义。
  - 断言探针**只能用代码里独有的串**：`uni.loadFontFace` 这类词在文件头注释里也会出现，拿它当探针会在每一端都命中，得到「没被剔除」的假失败（本项目已踩过两次）。
...
- **症状与危害**：
  ...
  - 但随着**分页数据不断追加、或者数据量超过数十条/数百条时**，使用 `<scroll-view>` 会导致原生 App 端（Android/iOS/鸿蒙）**内存呈线性无上限暴增**。底层所有的 DOM 节点及原生 View 全部被保留在渲染树中，垃圾回收（GC）极其频繁，滑动时出现**灾难性掉帧、卡顿、白屏**，严重时直接触发系统的 **OOM (Out Of Memory) 进程崩溃闪退**。
- **底层架构机制剖析**：
  1. **`<scroll-view>`：普通无复用滚动容器（Non-Virtualized Container）**：
     - `<scroll-view>` 本质是普通可滚动视口，内部所有的子项无论当前是否处于屏幕可见区内，**全部都会被完整创建并持续驻留在内存中**；
     - 随着列表无限追加，内存占用持续膨胀，CPU/GPU 每一帧都要维护极其庞大的层级树，无法应对海量或未知数量的数据流；
  2. **`<list-view>`：原生级虚拟复用列表组件（Virtualized Recycled List）**：
     - 在原生 App 端，`<list-view>` 底层基于操作系统的高性能列表组件实现（Android 端对应 `RecyclerView`，iOS 端对应 `UITableView` / `UICollectionView`）；
     - 具备完善的**视口外节点回收（Recycling）与视图复用池（View Pool）**机制：屏幕上滑出可视区域的 `<list-item>` 会立即被原生底层回收，并在新数据滑入屏幕时重新绑定数据复用该原生视图；
     - **无论列表加载了 100 条、1,000 条甚至 10,000 条数据，内存中始终只维持视口可见的极少数节点**，内存占用极低且恒定不变，能够维持稳定 60~120fps 的丝滑滑动体验。
- **强制使用与编码规范**：
  1. **必须使用 `<list-view>` 的场景**：
     - 任何支持分页加载、无限追加的瀑布流或列表（如商品列表、动态 Feed 流、资讯列表、聊天记录等）；
     - 数据量不确定或潜在可能超过 20 条的数据展示；
     - 业务推荐直接使用基于 `<list-view>` 构建的高级分页组件 `<z-paging-x>`（默认 `list-is="list-view"`）；
  2. **`<scroll-view>` 的合规场景（严禁用于长列表）**：
     - 有限短内容的整页或局部滚动（如系统设置页、详情页固定短表单）；
     - 顶部/横向滑动的分类筛选栏（`direction="horizontal"`）；
     - 局部固定尺寸弹窗内的短内容滚动；
  3. **`<list-view>` 结构规范**：
     - `<list-view>` 的直接子项**必须是 `<list-item>`**（或专用的 `sticky-header`）；
     - 业务卡片与数据项必须放置在 `<list-item>` 内部；
     - **严禁在 `<list-view>` 内部直接用一个外层 `<view>` 包裹所有子节点**（例如 `<list-view><view v-for="...">...</view></list-view>` 是严重错误写法，会导致原生复用池失效）。
...
- **症状与现象（本项目微信小程序 & 原生 App 实测）**：
  ...
  - 刷新尚未结束时用户点击底部 TabBar 切换到其他 Tab（如 Tab 1 基础）；
  - 当用户再次切换回首页时，**首页顶部一直卡在转圈动画中，永远不会停止，且后续无法再次触发刷新**。
- **底层架构机制剖析**：
  1. **`refresher-enabled` 动态切为 false 导致的「拔插头」状态机冻结**：
     - 若将 `isRefresherEnabled` 错误地绑定了 `isActive.value`，当切走 Tab 时，`isRefresherEnabled` 瞬间变成 `false`；
     - 微信小程序与原生 App（Android/iOS/鸿蒙）底层发现 `refresher-enabled: false` 后，会直接禁用下拉刷新模块手势并跳过收起动画，正在旋转的刷新控件被原样冻结在 DOM / 原生层中；
     - 当切回 Tab 时重新开启 `refresher-enabled: true`，但底层依然保留着上次未收起的展开节点，且因 `isTriggered` 保持为 `false`（无真实属性跳变），小程序与原生控件永远收不到收起信号；
  2. **不可见状态下的属性变更丢失（Virtual DOM Diff 吞噬）**：
     - 切走 Tab 时容器变为 `left: -9999px; visibility: hidden;`；
     - 在不可见状态下将 `isTriggered` 设为 `false`，部分端渲染层会跳过不可见节点的布局回流；
     - 切回时 `isTriggered.value` 依然为 `false`，Vue 3 的响应式根据 `hasChanged` 判定值未变化，不触发属性 patch，导致底层 native 控件永远保持旋转状态；
  3. **视图恢复时的残余 offset 误报触发**：
     - 切回可见瞬间，`scroll-view` 的尺寸与 offset 调整可能短时间内再次被小程序上报 `@refresherrefresh`，若不加拦截会再次错误开启 `isTriggered = true`。
- **治理与标准防御方案（三维一体）**：
  1. **解耦 `refresher-enabled` 与 `isActive`**：
     `isRefresherEnabled` 必须保持稳定（只由业务开关及全局滚动禁用开关决定），非激活 Tab 依靠 `left: -9999px; visibility: hidden;` 实现不可触摸，严禁切走时动态设为 false 拔掉插头；
  2. **绑定 `:key="refresherKey"` 强制彻底重构**：
     在刷新中途被切走中断时，记录 `wasRefreshInterrupted = true` 并递增 `refresherKey.value++`；无论是切出还是切回，通过全新组件实例彻底清空原生/小程序的卡死节点；
  3. **350ms 切 Tab 冷却期防残余误触发**：
     在 `handleRefresherRefresh` 中检查切 Tab 时间戳 `Date.now() - lastTabSwitchTime < 350`，短时间内收到的刷新事件坚决丢弃，彻底消除死锁闭环。
...
- **症状（本项目 Android 真机实测，RxJS 流式 Markdown 演示页）**：
  ...
  ```text
  java.lang.IllegalStateException: UTS instance 7 is not registered  [上报Bug] [AI修复]
    at io.dcloud.uniappxv.gen_bridge.GenBridgeCall.toInvocationParams
    at io.dcloud.uniappxv.bridge.Bridge.onJSTimerFired(Native Method)
    at Object.codec (<embedded>:20695:15)
    at Object.set (<embedded>:20853:28)
    at <embedded>:27040:41
    at RenderWatcherEffect.getter [as fn]
    ...
    at doWatch
  ```

  计时器驱动的流式渲染，会把某个**原生组件**写到「已释放的 UTS 实例」上，App 直接闪退。

- **通用定位手法（值得复用）**：
  1. **先从被写的字段反查组件**：栈里的 `codec` / `duration` 不是业务字段。到
     `unpackage/debug/*/Frameworks/DCloudUniappRuntime.framework/.../uni-app-x-framework-vapor-dev.js`
     里搜 `_codec`，能查到它们是 `<video>` 组件的**私有 props**，同一份 `setters` 里还有
     `element: UniNativeViewElement`、`videoPlayerView: UniVideoPlayerView`。
     关键推论：`codec` / `duration` **模板里根本没绑**，只有组件**初始化**时才会全量写一遍 ——
     所以崩溃点是「原生组件正在被**创建**」，不是被 patch。
  2. **用 `<embedded>:行` 反查源码行**：行号可直接对到 `unpackage/dist/dev/app-android/app-service.js`
     （先核对它的 mtime 是否等于崩溃时刻，本例 `15:26` 正好吻合）。本例
     `<embedded>:27040 / 27054 / 27056` 与 `uni_modules/mp-html/common/mermaid-lite/index.uts`
     的 584 / 600 / 609 逐行对齐，于是确认这一帧就在 mp-html 的**重渲染**里。
  3. 这一步能把「哪个组件 + 哪条渲染路径」钉死，避免在几百个文件里猜。

- **底层机制**：
  1. 分块到达时，末尾那个块（`<video>`、```` ```mermaid ````、`$$…$$`、表格）常常只到一半，
     此时**块结构还没确定**；
  2. 富文本解析器遇到「闭合标签还没到齐」的块级标签时，走的是 `closePos == -1 → continue`
     （既不推进游标、也不产出该块节点），于是它继续往后扫，把块**内部**的标签当**顶层节点**吐出来；
  3. 等闭合标签到齐，同一个标签又变成那个块的**子节点**，命中的是模板里**另一条渲染分支**；
  4. 原生组件（`<video>` / `<map>` / `<camera>` / `<web-view>`）于是在两条分支之间**搬家**：
     旧实例被释放、新实例被创建；重建时拿到的原生实例 id 已释放 →
     `UTS instance N is not registered`。每 40ms 重渲染一次，就反复触发。
     本例具体到 mp-html：`<video>` 会被 marked 包进 `<p>`，`</p>` 没到齐时它渲染在**顶层**
     video 分支，`</p>` 到齐后改由**嵌套** video 分支渲染。

- **治理：交给富文本渲染器的快照，块结构必须是完整的**
  - **只在块边界推进渲染快照**，半截块永不进渲染器：

    ```uts
    // 分块是 8 字符一块，空行大多落在块中间，所以取「最后一个完整块的结尾」，
    // 而不是判断整串是否以 \n\n 结尾 —— 那样会漏掉绝大多数块边界，流式会退化成几次大跳
    function lastBlockBoundary(text: string): number {
      const cut = text.lastIndexOf('\n\n')
      return cut < 0 ? -1 : cut + 2
    }

    // subscribe 里：接收与渲染分离
    receivedText = full
    const boundary = lastBlockBoundary(full)
    if (boundary > streamText.value.length) {
      streamText.value = full.substring(0, boundary)
    }

    // complete 回调里：把最后不足一个块边界的尾巴也渲染出来
    streamText.value = receivedText
    ```

  - **实测收益**（本项目 `DEMO_TEXT` 939 字符、8 字符/40ms，已用脚本重放校验）：
    重解析次数 **118 → 18**；每一份快照都以空行结尾；最终快照等于全文；长度严格递增不回退；
    `<video>` **首现即完整**，其之前的前缀此后不再变形。
  - **反例**：`subscribe` 里直接 `streamText.value = full`（每个 chunk 都重渲染）
    —— 这就是崩溃的写法。
  - **适用范围**：不限于 Markdown。任何「计时器高频驱动 + 内容里含原生组件」的场景
    （SSE 打字机、日志流、实时表格）都成立。
...
- **前提：只有蒸汽模式认这个属性**，VDOM 模式不支持（写了不生效）。本项目 `manifest.json` 是 `"vapor": true` + `"vapor-render-target": "bytecode"`（HEAD 默认，见 1.3.20），编译器 5.24，版本门槛已满足：
  ...
  | --- | --- | --- | --- |
  | 拍平引入版本 | 5.21 | 5.11 | 5.0 |

- **红线一：拍平的元素不响应事件**（`click`、`touch` 全都没有），也不支持截图 API `takeSnapshot`、不支持 `pointer-events`。→ **凡是挂了 `@click` / `@tap` / `@touchstart` / `@touchmove` 的元素，一个字都不要加**。失败是**静默**的：编译不报错、运行不打警告，只是点了没反应 —— 排查时极难往「属性写多了」上想。

- **红线二：一批 CSS 随之失效**，**同样静默**：
  - 不支持 `visibility`、`z-index`、`background-image`、`background-clip`、`pointer-events`；
  - `box-shadow` 不支持 `inset` 与 `spreadRadius`；
  - `display: fixed` 有缺陷：无法覆盖其它非 fixed 元素；
  - `transition` 不支持渲染类样式（如 `background-color` 的过渡）；
  - **`view` 拍平后 `transform` / `overflow` / `display` / `opacity` 有缺陷**：**当该 view 有子元素时**，这些样式不会作用到非拍平的子元素上（**无子元素时不踩这条**）；
  - **`image` 拍平后不能播动画**：gif / webp 只显示第一帧。

- **红线三：它是初始化属性，不支持动态修改**。模板里静态写 `flatten`，不要绑变量做条件拍平（`v-if` 换分支可以，`:flatten="cond"` 不行）。

- **自定义组件会自动透传**：自定义组件的**单根节点**是 view / text / image 时，该组件自动支持 `flatten` 并把它透传给根节点；不符合这个结构时写了会被**自动忽略** —— 又一处静默失败，别以为加上就生效了。

- **鸿蒙特例（唯一「加了反而更慢」的平台）**：逻辑上都可以设为 true，但**只有至少两个「相邻元素」同时拍平才有收益，单独拍平可能导致性能下降**。相邻包括：兄弟节点（不要求紧挨，同一父下任意两个及以上子元素拍平即可）、父子节点（要求**直接紧邻** —— `P→C→G` 里只拍 `P` 和 `G`、中间隔了 `C` 则无效）、以及两者的混合。→ **鸿蒙上要么成片加，要么一个都别加。**
