...
function doLogin() {
  if (tokenStore.hasValidLogin()) {
    if (redirectUrl.value !== '') {
      const targetUrl = UTSAndroid.consoleDebugError(decodeURIComponent(redirectUrl.value), " at src/sub/auth/login.uvue:61") ?? ''
  ...
  setTimeout(() => {
    if (redirectUrl.value !== '') {
      const targetUrl = UTSAndroid.consoleDebugError(decodeURIComponent(redirectUrl.value), " at src/sub/auth/login.uvue:96") ?? ''
      if (isPageTabbar(targetUrl)) {
        setCurIdxByPath(targetUrl)
        uni.switchTab({ url: targetUrl })
      }
