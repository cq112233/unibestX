...
const formRules = { __$originalPosition: new UTSSourceMapPosition("formRules", "src/sub/uview-ultra/components/DemoForm.uvue", 208, 7), 
  name: [
    {
      required: true,
      message: '请填写姓名',
...
function submitForm() {
  ...
  if (form != null) {
    (form.$callMethod('validate') as Promise<any>).then((res: any) => {
      ...
      console.log('提交的数据：', formModel, " at src/sub/uview-ultra/components/DemoForm.uvue:258")
    }).catch((err: any | null) => {
      console.log('验证失败:', err, " at src/sub/uview-ultra/components/DemoForm.uvue:260")
      uni.showToast({
        title: '请检查必填项',
        icon: 'error',
      })
...
const _component_up_action_sheet = resolveEasyComponent("up-action-sheet",_easycom_up_action_sheet)
  ...
  return _cE("view", null, [
    _cE("text", _uM({ class: "section-title" }), " 完整表单演示 "),
