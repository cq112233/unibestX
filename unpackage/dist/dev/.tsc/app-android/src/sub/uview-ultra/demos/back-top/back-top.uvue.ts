            ...
            _cE("scroll-view", _uM({
              ...
              _cE("view", _uM({ class: "bg-white rounded-8px p-12px mb-12px shadow-xs" }), [
                _cE("view", _uM({ class: "flex-row items-center mb-10px" }), [
                  ...
                  _cE("text", _uM({ class: "text-14px font-bold text-_b_h1e293b_B" }), "返回顶部说明")
                ]),
                _cE("text", _uM({ class: "text-13px text-_b_h64748b_B leading-20px" }), " 请向下滑动页面（超过 100px），右下角会自动浮现返回顶部按钮，点击后平滑一键置顶。 "),
                _cE("text", _uM({ class: "text-12px text-_b_h2563eb_B mt-6px" }), "当前 实时高度: " + _tD(currentScrollTop.value) + "px", 1 /* TEXT */)
              ]),
              _cE(Fragment, null, RenderHelpers.renderList(20, (i, __key, __index, _cached): any => {
                return _cE("view", _uM({
                  ...
                  _cE("text", _uM({ class: "text-14px font-bold text-_b_h334155_B" }), "长列表测试卡片 #" + _tD(i), 1 /* TEXT */),
                  _cE("text", _uM({ class: "text-12px text-_b_h94a3b8_B mt-4px" }), "向下滚动触发右下角返回顶部悬浮图标...")
                ])
              }), 64 /* STABLE_FRAGMENT */)
