            ...
            _cE("scroll-view", _uM({
              ...
              _cE("view", _uM({ class: "bg-white rounded-8px p-12px mb-12px shadow-xs" }), [
                _cE("view", _uM({ class: "flex-row items-center mb-10px" }), [
                  _cE("view", _uM({ class: "w-3px h-14px bg-_b_h2979ff_B mr-6px rounded-2px" })),
                  _cE("text", _uM({ class: "text-14px font-bold text-_b_h1e293b_B" }), "吸顶效果说明")
                ]),
                _cE("text", _uM({ class: "text-13px text-_b_h64748b_B leading-20px" }), " 向上滑动页面，当下面的操作栏滚动到顶部时，会触发 up-sticky 组件吸顶悬浮固定。 ")
              ]),
              _cV(_component_up_sticky, _uM({
                ...
                default: withSlotCtx((): any[] => [
                  _cE("view", _uM({ class: "bg-_b_h2979ff_B p-12px rounded-8px flex-row items-center justify-between shadow-sm mb-10px" }), [
              ...
              _cE(Fragment, null, RenderHelpers.renderList(25, (i, __key, __index, _cached): any => {
                return _cE("view", _uM({
                  ...
                  _cE("text", _uM({ class: "text-14px font-bold text-_b_h334155_B" }), "列表条目 #" + _tD(i), 1 /* TEXT */),
                  _cE("text", _uM({ class: "text-12px text-_b_h94a3b8_B mt-4px" }), "向下滑动页面查看吸顶保持效果...")
                ])
              }), 64 /* STABLE_FRAGMENT */)
