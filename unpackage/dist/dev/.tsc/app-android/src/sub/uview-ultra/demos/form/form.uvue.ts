...
const formRules = { __$originalPosition: new UTSSourceMapPosition("formRules", "src/sub/uview-ultra/demos/form/form.uvue", 224, 7), 
  name: [
    {
      required: true,
      message: '请填写姓名',
...
function submitForm() {
  ...
  if (form != null) {
    (form.$callMethod('validate') as Promise<any>).then((res: any) => {
      ...
      console.log('提交的数据：', formModel, " at src/sub/uview-ultra/demos/form/form.uvue:274")
    }).catch((err: any | null) => {
      console.log('验证失败:', err, " at src/sub/uview-ultra/demos/form/form.uvue:276")
      uni.showToast({
        title: '请检查必填项',
        icon: 'error',
      })
