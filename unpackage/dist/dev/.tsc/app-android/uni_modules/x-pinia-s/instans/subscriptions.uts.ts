	...
	trigger(mutation : SubscriptionMutation, state : UTSJSONObject) : void {
		...
		for (let i = 0; i < snapshot.length; i++) {
			try {
				...
				__f__('warn','at uni_modules/x-pinia-s/instans/subscriptions.uts:54','[x-pinia-s] state subscription error:', e)
			}
		}
	}
	...
	trigger(ctx : ActionContext) : ActionTriggerResult {
		...
		for (let i = 0; i < snapshot.length; i++) {
			try {
				...
				__f__('warn','at uni_modules/x-pinia-s/instans/subscriptions.uts:118','[x-pinia-s] action subscription error:', e)
			}
		}
