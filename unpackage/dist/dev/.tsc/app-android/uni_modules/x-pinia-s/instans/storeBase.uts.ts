...
export class PiniaStoreBase {
	...
	protected bindState(state : any) : void {
		if (this._bound) {
			__f__('warn','at uni_modules/x-pinia-s/instans/storeBase.uts:106','[x-pinia-s][' + this.$id + '] bindState 只能调用一次')
			return
		}
		this._bound = true
		this._state = state
	...
	private _invokeAction(name : string, fn : () => any, args : Array<any>) : any | null {
		...
		try {
			...
			for (let i = 0; i < triggerResult.errorList.length; i++) {
				try {
					...
					__f__('warn','at uni_modules/x-pinia-s/instans/storeBase.uts:179','[x-pinia-s][' + this.$id + '] onError callback error:', e2)
				}
			}
			throw e
		}
		for (let i = 0; i < triggerResult.afterList.length; i++) {
			try {
				...
				__f__('warn','at uni_modules/x-pinia-s/instans/storeBase.uts:188','[x-pinia-s][' + this.$id + '] after callback error:', e3)
			}
		}
		return result
	}
	...
	$dispose() : void {
		if (this._watchStopper != null) {
			try {
				...
				__f__('warn','at uni_modules/x-pinia-s/instans/storeBase.uts:244','[x-pinia-s][' + this.$id + '] watch stop error:', e)
			}
		...
		if (this._scope != null) {
			try {
				...
				__f__('warn','at uni_modules/x-pinia-s/instans/storeBase.uts:255','[x-pinia-s][' + this.$id + '] scope stop error:', e)
			}
			this._scope = null
		}
		if (this._pinia != null) {
