...
export function defineStore<T>(
	...
	return () : T => {
		...
		if (cached != null) {
			__f__('log','at uni_modules/x-pinia-s/instans/defineStore.uts:57','[x-pinia-s] reuse cached store:', id)
			return cached as any as T
		}
		__f__('log','at uni_modules/x-pinia-s/instans/defineStore.uts:60','[x-pinia-s] create new store:', id)
