...
export function upGetRect(selector: string, all: boolean = false, comp: ComponentPublicInstance|ComponentInternalInstance|null = null): Promise<NodeInfo> {
	return new Promise((resolve) => {
		try {
			const query = uni.createSelectorQuery()
			const queryIn = comp != null ? query.in(comp) : query
			if (all == true) {
				queryIn.selectAll(selector)
					.boundingClientRect((res: any | null) => {
						if (res != null && Array.isArray(res)) {
							const arr = res as NodeInfo[]
							resolve(arr.length > 0 ? arr[0] : ({} as NodeInfo))
						} else if (res != null) {
...
export function upGetAllRect(selector: string, comp: ComponentPublicInstance|ComponentInternalInstance|null = null): Promise<Array<NodeInfo>> {
	return new Promise((resolve) => {
		try {
			const query = uni.createSelectorQuery()
			const queryIn = comp != null ? query.in(comp) : query
...
/**
   ...
   */
export function bem(name: string, fixed: Array<string>, change: Array<Array<any>>): string  {
	  // 类名前缀
 ...
 */
export function sleep(value = 30): Promise<UTSJSONObject> {
    return new Promise((resolve) => {
        setTimeout(() => {
            resolve({})
        }, value)
...
export function addStyle(customStyle: any, target = 'object'): any {
    if (testEmpty(customStyle)) {
        return target === 'object' ? ({} as UTSJSONObject) : ''
    }
    ...
    // 字符串转对象
    if (target === 'object') {
        let customStyleStr = typeof customStyle === 'string' ? (customStyle as string) : JSON.stringify(customStyle)
        // 去除字符串样式中的两端空格
    ...
    if (typeof customStyle === 'object' && customStyle != null) {
        UTSJSONObject.keys(customStyle as UTSJSONObject).forEach((key: string) => {
 ...
 */
export function addUnit(pvalue: any|null, punit: string|null = ''): string {
	let value = pvalue
	if (value == null) {
		value = ''
 ...
 */
export function deepClone(obj: object): any|null {
    // 对常见的“非”值，直接返回原来值
...
export function error(err: string): void {
    ...
    if (process.env.NODE_ENV === 'development') {
        __f__('error','at uni_modules/uview-ultra/libs/function/index.uts:361',`uview-plus提示：${err}`)
    }
}
 ...
 */
 export function timeFormat(dateTime: any|null = 0, formatStr: string = 'yyyy-mm-dd'): string {
  let date: any
    // 若传入时间为假值，则取当前时间
  ...
  // __f__('log','at uni_modules/uview-ultra/libs/function/index.uts:450',timeSource)
  for (const key in timeSource) {
    const ret = new RegExp(`${key}+`).exec(formatStr)
    if (ret != null && ret[0] != null) {
	  // __f__('log','at uni_modules/uview-ultra/libs/function/index.uts:454','ret', ret)
	  // 年可能只需展示两位
	  const beginIndex = key === 'y' && ret.length == 2 ? 2 : 0
	  if (timeSource[key] != null
		&& key != '') {
		let replaceVal: string = ''
		let timeSourceKey = timeSource.getString(key)
		// __f__('log','at uni_modules/uview-ultra/libs/function/index.uts:461','timeSourceKey', typeof timeSourceKey)
...
//  */
// export function timeFrom(timestamp: number = 0, format: string = 'yyyy-mm-dd'): string {
 ...
 */
export function queryParams(data: UTSJSONObject = {}, isPrefix: boolean = true, arrayFormat: string = 'brackets'): string {
	return ''
 //    const prefix = isPrefix ? '?' : ''
 //    const _result: string[] = []
 //    if (['indices', 'brackets', 'repeat', 'comma'].indexOf(arrayFormat) == -1) arrayFormat = 'brackets'
	// data.toMap().forEach((value: any | null, key: string) => {
 //        // 去掉为空的参数
 ...
 */
export function toast(title: string|null, duration = 2000): void {
 ...
 */
export function type2icon(type: string = 'success', fill: boolean = false): string {
    // 如果非预置值,默认为success
    ...
    if (fill) iconName = iconName + '-fill'
    return iconName
}
 ...
 */
export function padZero(value: any): string {
...
//  * @returns {*}
//  */
export function getProperty(obj: UTSJSONObject|null, key: string): any {
    if (null == obj) {
        return ''
    }
    if (key === '') {
    ...
    if (key.indexOf('.') != -1) {
        ...
        let currentObj: UTSJSONObject = obj
        for (let i = 0; i < keys.length; i++) {
            const value: any = currentObj[keys[i]] ?? ''
            if (i == keys.length - 1) {
                return value
            }
...
function setNestedValue(obj: any | null, keys: string[], index: number, value: any): void {
    if (obj == null) {
        return
    }
    const k = keys[index]
...
export function page(): string {
	...
	let url = ''
	// if (pages[pages.length - 1].route != null) {
 ...
 */
export function pages(): Array<UniNormalPage> {
    const pages: Array<UniNormalPage> = getCurrentPages()
    return pages
}

export function getParentFunc(name = '', instance: ComponentPublicInstance): ComponentPublicInstance | null {
    let parentTmp = instance.$parent
    // 通过while遍历，这里主要是为了H5需要多层解析的问题
...
export function getParentDataFunc(that: any, parentName: string): any {
	__f__('log','at uni_modules/uview-ultra/libs/function/index.uts:884','that', JSON.stringify(that))
    ...
    // __f__('log','at uni_modules/uview-ultra/libs/function/index.uts:892','that.parent', that.parent)
    // todo
