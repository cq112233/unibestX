...
export function useUltraUI(parentDataInit: UTSJSONObject = {}): UPUseUltraUIType  {
    ...
    const getChildIndex = function(ins: ComponentPublicInstance): number {
        ...
        if (parent.value != null) {
            ...
            let childs = parentIns.$callMethod('getChildren') as ComponentPublicInstance[] | null
            if (childs == null) {
                childs = parentIns.$data['children'] as ComponentPublicInstance[] | null
            }
            if (childs != null) {
                childs.forEach((child: ComponentPublicInstance, idx: number) => {
                    if (child == ins) {
                        index = idx
                    }
                })
    ...
    const getParentData = function(parentName: string, instance: ComponentPublicInstance, refMode: boolean): ComponentPublicInstance|null {
		...
		// __f__('log','at uni_modules/uview-ultra/libs/composable/useUltraUI.uts:128','parentIns', parentIns)
        ...
        if (parentIns != null) {
            ...
            UTSJSONObject.keys(parentData.value).map((key: string) => {
                const propsData = parentIns?.$callMethod('getProps') as UTSJSONObject | null
                if (propsData != null && UTSJSONObject.keys(propsData).includes(key)) {
                    parentData.value[key] = propsData[key]
                }

                const refsData = parentIns?.$callMethod('getRefs') as UTSJSONObject | null
