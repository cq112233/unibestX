    ...
    methods: {
	    ...
	    */
        ...
        getParent(name: string): ComponentPublicInstance|null {
            ...
            while (parent != null) {
            	// __f__('log','at uni_modules/uview-ultra/libs/mixin/mixin.uts:107',parent.$options)
                // 父组件
                if (
					// parent.$options != null &&
		...
		addChild(ins: ComponentPublicInstance): void {
			...
			childs.map((child: ComponentPublicInstance) => {
				// __f__('log','at uni_modules/uview-ultra/libs/mixin/mixin.uts:126','&&&&&', child)
				if (ins == child) {
					exist = true
				}
			})
			if (exist == false) {
				// __f__('log','at uni_modules/uview-ultra/libs/mixin/mixin.uts:132','ins&&&&&&&&&', ins)
				// @ts-ignore
