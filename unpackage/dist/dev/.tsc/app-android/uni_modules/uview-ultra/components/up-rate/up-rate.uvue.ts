	...
	const __sfc__ = defineComponent({
		...
		mounted() {
			const modelVal: number = this.modelValue == null ? 0 : parseFloat(this.modelValue.toString())
			const minCount: number = this.minCount == null ? 0 : parseFloat(this.minCount.toString())
		...
		methods: {
			...
			getRateIconWrapRect() {
				// @ts-ignore
				this.$uGetRect("." + this.elClass).then((res: NodeInfo) => {
			...
			// 通过点击，直接选中
			clickHandler(e: any, index: number) {
			...
			// 获取当前激活的评分图标
			getActiveIndex(x: number, isClick = false) {
...
function GenUniModulesUviewUltraComponentsUpRateUpRateRender(this: InstanceType<typeof __sfc__>): any | null {
const _ctx = this
...
const _component_up_icon = resolveEasyComponent("up-icon",_easycom_up_icon)
  ...
  return _cE("view", _uM({
    ...
    _cE("view", _uM({
      ...
      onTouchmove: withModifiers(_ctx.touchMove, ["stop"]),
      onTouchend: withModifiers(_ctx.touchEnd, ["stop"])
    }), [
      _cE(Fragment, null, RenderHelpers.renderList(parseInt(_ctx.count.toString()), (item, index, __index, _cached): any => {
        return _cE("view", _uM({
          class: _nC(["up-rate__content__item cursor-pointer", [_ctx.elClass]]),
          key: index
        }), [
          _cE("view", _uM({
            ...
            onClick: withModifiers(($event: any) => {_ctx.clickHandler($event, index + 1)}, ["stop"])
          }), [
            _cV(_component_up_icon, _uM({
              name: 
                            Math.floor(parseFloat(_ctx.activeIndex.toString())) > index
                                ? _ctx.activeIcon
                                : _ctx.inactiveIcon
                        ,
              color: 
                        ...
                        ,
              "custom-style": {
                            padding: `0 ${_ctx.addUnit(parseInt(_ctx.gutter.toString()) / 2)}`,
                        },
              size: _ctx.size
          ...
          isTrue(_ctx.allowHalf)
            ? _cE("view", _uM({
                ...
                style: _nS([_uM({
                        width: _ctx.addUnit(_ctx.rateWidth / 2),
                    })]),
                ref_for: true,
                ref: "up-rate__content__item__icon-wrap"
              }), [
                _cV(_component_up_icon, _uM({
                  ...
                  color: 
                            _ctx.disabled
                                ? '#c8c9cc'
                                : Math.ceil(parseFloat(_ctx.activeIndex.toString())) > index
                                ? (_ctx.activeColor === '#FA3534' ? 'primary' : _ctx.activeColor)
                                : _ctx.inactiveColor
                        ,
                  "custom-style": {
    ...
    ], 40 /* PROPS, NEED_HYDRATION */, ["onTouchmove", "onTouchend"])
  ], 12 /* STYLE, PROPS */, ["id"])
}
export type UpRateComponentPublicInstance = InstanceType<typeof __sfc__>;
const GenUniModulesUviewUltraComponentsUpRateUpRateStyles = [_uM([["up-rate", _pS(_uM([["display", "flex"], ["flexDirection", "row"], ["alignItems", "center"], ["marginTop", 0], ["marginRight", 0], ["marginBottom", 0], ["marginLeft", 0], ["paddingTop", 0], ["paddingRight", 0], ["paddingBottom", 0], ["paddingLeft", 0]]))], ["up-rate__content", _pS(_uM([["display", "flex"], ["flexDirection", "row"]]))], ["up-rate__content__item", _pS(_uM([["position", "relative"]]))], ["up-rate__content__item__icon-wrap--half", _pS(_uM([["position", "absolute"], ["overflow", "hidden"], ["top", 0], ["left", 0]]))], ["up-icon", _pS(_uM([["boxSizing", "border-box"]]))]])]
