	...
	import { propsTabbar } from './props.uts';
	...
	const __sfc__ = defineComponent({
		...
		computed: {
			tabbarStyle(): any {
				const style = {__$originalPosition: new UTSSourceMapPosition("style", "uni_modules/uview-ultra/components/up-tabbar/up-tabbar.uvue", 60, 11),
					zIndex: this.zIndex
				}
				if (this.borderColor != '') {
		...
		methods: {
			...
			async setPlaceholderHeight() {
				...
				await sleep(20)
				// @ts-ignore
				this.upGetRect('.up-tabbar__content').then((size: NodeInfo) => {
...
function GenUniModulesUviewUltraComponentsUpTabbarUpTabbarRender(this: InstanceType<typeof __sfc__>): any | null {
const _ctx = this
...
const _component_up_safe_bottom = resolveEasyComponent("up-safe-bottom",_easycom_up_safe_bottom)
  ...
  return _cE("view", _uM({ class: "up-tabbar" }), [
    ...
    isTrue(_ctx.placeholder)
      ? _cE("view", _uM({
          key: 0,
          class: "up-tabbar__placeholder",
          style: _nS(_uM({
				height: _ctx.placeholderHeight + 'px',
			}))
        }), null, 4 /* STYLE */)
      : _cC("v-if", true)
  ])
