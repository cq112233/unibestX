...
// IntersectionObserver 引用（仅 H5 / MP 平台使用）




// 计算属性：图片高度，可能为auto，带%，或者直接数值
...
const wrapStyle = computed(() => {
    const br = props.borderRadius != null ? props.borderRadius.toString() : '0'
    const t = parseFloat(time.value.toString())
    return {
        opacity: opacity.value,
        borderRadius: br + 'rpx',
        transition: 'opacity ' + (t / 1000).toString() + 's ease-in-out'
    }
})
...
watch(isShow, (nVal: boolean) => {
    // 如果是不开启过渡效果，直接返回
    if (props.isEffect == false) return
    time.value = 0
...
// 监听 image 变化：重新标记状态
const imageValue = computed(() => props.image)
watch(imageValue, (n: string) => {
    if (n == '') {
...
// 点击图片触发的事件
// loadlazy-还是懒加载中状态，loading-图片正在加载，loaded-图片加载完成
function clickImg() {
    let whichImg = ''
    // 如果isShow为false，意味着图片还没开始加载，点击的只能是最开始的占位图
    if (isShow.value == false) whichImg = 'lazyImg'
    // 如果isError为true，意味着图片加载失败，这是只剩下错误的占位图
    else if (isError.value == true) whichImg = 'errorImg'
...
onMounted(() => {
    ...
    nextTick(() => {
        uni.$once('uOnReachBottom', (arg: any) => {
            if (isShow.value == false) isShow.value = true
        })
    })
    ...
    setTimeout(() => {
        ...
        // App 原生端不支持 IntersectionObserver，直接展示
        isShow.value = true
...
return (): any | null => {
  ...
  return _cE("view", _uM({
    class: _nC(["up-wrap", 'up-lazy-item-' + unref(elIndex)]),
    style: _nS(wrapStyle.value)
  }), [
    _cE("view", _uM({
      class: _nC('up-lazy-item-' + unref(elIndex))
    }), [
      isTrue(!isError.value)
        ? _cE("image", _uM({
            ...
            class: "up-lazy-item",
            src: isShow.value ? _ctx.image : _ctx.loadingImg,
            mode: _ctx.imgMode,
            onLoad: imgLoaded,
            onError: loadError,
            onClick: clickImg
          }), null, 44 /* STYLE, PROPS, NEED_HYDRATION */, ["src", "mode"])
        : _cE("image", _uM({
            key: 1,
            style: _nS(imageStyle.value),
