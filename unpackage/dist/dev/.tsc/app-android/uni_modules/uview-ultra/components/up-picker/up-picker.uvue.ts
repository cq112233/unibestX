 ...
 */
import { propsPicker } from './props';
import { mpMixin } from '../../libs/mixin/mpMixin';
import { mixin } from '../../libs/mixin/mixin';
import { addUnit, deepClone, sleep } from '../../libs/function/index';
import { array as testArray, object as testObject } from '../../libs/function/test';
const __sfc__ = defineComponent({
	...
	data() {
		return {
			...
			innerIndex: [] as number[],
			// 各列的值
			innerColumns: [] as Array<Array<any>>,
			// 上一次的变化列索引
			columnIndex: 0,
    ...
    computed: {
		pickerViewHeight(): string {
			// @ts-ignore
			let tmp = parseInt(this.visibleItemCount.toString())
			// @ts-ignore
				* parseInt(this.itemHeight.toString())
			return addUnit(tmp)
		},
        inputLabel(): string[] {
			let items: Array<any> = []
	...
	methods: {
		...
		testArray(obj: any): boolean {
			return testArray(obj);
		},
		// 获取item需要显示的文字，判别为对象还是文本
		getItemText(item: any): any|null {
			if (typeof item == 'object') {
				let ele1 = (item as UTSJSONObject)[this.keyName]
				if (ele1 != null) {
		...
		// 选择器某一列的数据发生变化时触发
		changeHandler(e: UniPickerViewChangeEvent): void {
			const {
				value
			} = e.detail
			let index = 0,
		...
		setIndexs(index: number[] | null, setLastIndex: boolean): void {
		...
		// 设置整体各列的columns的值
		setColumns(columns: Array<Array<any>>): void {
			// console.log(columns)
		...
		getValues(): Array<any> {
			...
			// })()
			let res: Array<any> = []
			return this.innerColumns.map((item: Array<any>, index) => {
...
function GenUniModulesUviewUltraComponentsUpPickerUpPickerRender(this: InstanceType<typeof __sfc__>): any | null {
const _ctx = this
...
const _component_up_popup = resolveEasyComponent("up-popup",_easycom_up_popup)
  ...
  return _cE("view", _uM({ class: "up-picker-warrper" }), [
    isTrue(_ctx.hasInput)
      ? _cE("view", _uM({
          key: 0,
          class: "up-picker-input cursor-pointer",
          onClick: () => {_ctx.showByClickInput = !_ctx.showByClickInput}
        }), [
          renderSlot(_ctx.$slots, "default", {}, (): any[] => [
            _cE("view", null, _tD(_ctx.inputLabel.length > 0 ? _ctx.inputLabel.join('/') : _ctx.placeholder), 1 /* TEXT */)
          ])
        ], 8 /* PROPS */, ["onClick"])
      : _cC("v-if", true),
    _cV(_component_up_popup, _uM({
      show: _ctx.pageInline || _ctx.show || (_ctx.hasInput && _ctx.showByClickInput),
      mode: _ctx.popupMode,
        ...
        _cE("view", _uM({ class: "up-picker" }), [
          isTrue(_ctx.showToolbar)
            ? _cV(_component_up_toolbar, _uM({
                ...
                _: 3 /* FORWARDED */
              }), 8 /* PROPS */, ["cancelColor", "confirmColor", "cancelText", "confirmText", "title", "rightSlot", "onCancel", "onConfirm"])
            : _cC("v-if", true),
          renderSlot(_ctx.$slots, "toolbar-bottom"),
          _cV(_component_picker_view, _uM({
            class: "up-picker__view",
            indicatorStyle: `height: ${_ctx.addUnit(_ctx.itemHeight)}`,
            value: _ctx.innerIndex,
            immediateChange: _ctx.immediateChange,
            style: _nS(_uM({
						height: _ctx.pickerViewHeight
					})),
                    ...
                    isTrue(_ctx.testArray(item))
                      ? _cE(Fragment, _uM({ key: 0 }), RenderHelpers.renderList(item, (item1, index1, __index, _cached): any => {
                          return _cE("view", _uM({
                            ...
                            key: index1,
                            style: _nS(_uM({
								height: _ctx.addUnit(_ctx.itemHeight)
							}))
                          }), [
                            _cE("text", _uM({
                              class: "up-picker__view__column__item__text",
                              style: _nS(_uM({
									lineHeight: _ctx.addUnit(_ctx.itemHeight),
									fontWeight: (_ctx.innerIndex.length -1  >= index && index1 === _ctx.innerIndex[index]) ? 'bold' : 'normal',
								}))
