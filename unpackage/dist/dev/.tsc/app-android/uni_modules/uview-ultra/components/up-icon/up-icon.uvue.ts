...
__ins.emit(event, ...do_not_transform_spread)
	...
	const iconStyle = computed(() => {
		let style = {__$originalPosition: new UTSSourceMapPosition("style", "uni_modules/uview-ultra/components/up-icon/up-icon.uvue", 160, 7),
	...
	const imgStyle = computed(() => {
		let style = {__$originalPosition: new UTSSourceMapPosition("style", "uni_modules/uview-ultra/components/up-icon/up-icon.uvue", 182, 7),}
	...
	const mergedImgStyle = computed((): UTSJSONObject => {
		const custom = addStyle(props.customStyle) as UTSJSONObject
		const img = imgStyle.value as UTSJSONObject
		return deepMerge(img, custom) as UTSJSONObject
	})

	const mergedIconStyle = computed((): UTSJSONObject => {
		const custom = addStyle(props.customStyle) as UTSJSONObject
		const iconS = iconStyle.value as UTSJSONObject
		return deepMerge(iconS, custom) as UTSJSONObject
	})

	const labelStyleComputed = computed((): UTSJSONObject => {
		return {
  ...
  return _cE("view", _uM({
    ...
    _ctx.label !== ''
      ? _cE("text", _uM({
          ...
          style: _nS(labelStyleComputed.value)
