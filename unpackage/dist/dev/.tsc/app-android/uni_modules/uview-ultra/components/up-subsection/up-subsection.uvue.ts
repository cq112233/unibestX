...
const __sfc__ = defineComponent({
    ...
    computed: {
        wrapperStyle() {
            const style = {__$originalPosition: new UTSSourceMapPosition("style", "uni_modules/uview-ultra/components/up-subsection/up-subsection.uvue", 109, 19),};
            // button模式时，设置背景色
            if (this.mode === "button") {
        ...
        // 滑块的样式
        barStyle(): any {
            const style = {__$originalPosition: new UTSSourceMapPosition("style", "uni_modules/uview-ultra/components/up-subsection/up-subsection.uvue", 118, 19),};
            if (this.componentWidth > 0) {
                const itemWidth = this.componentWidth / this.list.length;
                style['width'] = itemWidth.toString() + 'px';
                style['wtu-hew3f9-0'] = `translateX(${this.innerCurrent * itemWidth}px)`;
            } else {
                if (this.list.length > 0) {
                    style['width'] = (100.0 / this.list.length).toString() + '%';
                }
                style['wtu-hew3f9-0'] = `translateX(${this.innerCurrent * 100}%)`;
            }
            if (this.firstTime) {
        ...
        innerBarStyle(): any {
            const style = {__$originalPosition: new UTSSourceMapPosition("style", "uni_modules/uview-ultra/components/up-subsection/up-subsection.uvue", 140, 19),};
    ...
    methods: {
		...
		itemStyle(index: number) {
			const style = {__$originalPosition: new UTSSourceMapPosition("style", "uni_modules/uview-ultra/components/up-subsection/up-subsection.uvue", 204, 10),};
			if (this.mode === "subsection") {
				// 设置border的样式
				if (this.activeColor != '#3c9cff') {
		...
		textStyle(index: number) {
			const style = {__$originalPosition: new UTSSourceMapPosition("style", "uni_modules/uview-ultra/components/up-subsection/up-subsection.uvue", 216, 10),};
			if (this.disabled) {
		...
		getText(item: any|null): string|null {
			if (typeof item === 'string') {
				...
				let temp = UTSAndroid.consoleDebugError(JSON.parse(JSON.stringify(item)), " at uni_modules/uview-ultra/components/up-subsection/up-subsection.uvue:257") as UTSJSONObject;
