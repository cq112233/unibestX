...
const lineStyle = computed<UTSJSONObject>(() => {
  const style = { __$originalPosition: new UTSSourceMapPosition("style", "uni_modules/uview-ultra/components/up-swiper-indicator/up-swiper-indicator.uvue", 74, 9), } as UTSJSONObject
...
function dotStyle(index: number): UTSJSONObject {
  const style = { __$originalPosition: new UTSSourceMapPosition("style", "uni_modules/uview-ultra/components/up-swiper-indicator/up-swiper-indicator.uvue", 89, 9), } as UTSJSONObject
  ...
  return _cE("view", _uM({ class: "up-swiper-indicator" }), [
    _ctx.indicatorMode === 'line'
      ? _cE("view", _uM({
          ...
          style: _nS(lineWrapperStyle.value)
        }), [
          _cE("view", _uM({
    ...
    _ctx.indicatorMode === 'dot'
      ? _cE("view", _uM({
          ...
          _cE(Fragment, null, RenderHelpers.renderList(lengthArray.value, (item, index, __index, _cached): any => {
            return _cE("view", _uM({
              class: _nC(["up-swiper-indicator__wrapper__dot", [index === parseInt(_ctx.current.toString()) ? 'up-swiper-indicator__wrapper__dot--active' : '']]),
              key: index,
              style: _nS(dotStyle(index))
            }), null, 6 /* CLASS, STYLE */)
          }), 128 /* KEYED_FRAGMENT */)
        ])
      : _cC("v-if", true)
