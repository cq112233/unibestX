	...
	const __sfc__ = defineComponent({
		...
		data() {
			return {
				...
				monthsInner: [] as Array<CalendarMonthItem>,
		...
		computed: {
			...
			dayStyle() {
				return (index1: number, index2: number, item: UPCalendarMonthsItemDate):any => {
					const style = { __$originalPosition: new UTSSourceMapPosition("style", "uni_modules/uview-ultra/components/up-calendar/month.uvue", 180, 12), } as UTSJSONObject
					if (item != null) {
			...
			daySelectStyle() {
				return (index1: number, index2: number, item: UPCalendarMonthsItemDate) => {
					let date = (item['dateStr'] != null ? item['dateStr'] as string : dayuts(item['date']).format("YYYY-MM-DD")),
						style = {__$originalPosition: new UTSSourceMapPosition("style", "uni_modules/uview-ultra/components/up-calendar/month.uvue", 200, 7),}
			...
			textStyle() {
				return (item: UPCalendarMonthsItemDate) => {
					const date = (item['dateStr'] != null ? item['dateStr'] as string : dayuts(item['date']).format("YYYY-MM-DD")),
						style = {__$originalPosition: new UTSSourceMapPosition("style", "uni_modules/uview-ultra/components/up-calendar/month.uvue", 261, 7),}
					// 选中的日期，提示文字设置白色
					const sameDate = (element: string): boolean => this.dateSame(element, date)
					if (this.selected.some(sameDate)) {
						style['color'] = '#ffffff'
		...
		methods: {
			getMonthTitle(item: CalendarMonthItem): string {
			...
			rangeBgStyle(item: UPCalendarMonthsItemDate): any {
				const style = { __$originalPosition: new UTSSourceMapPosition("style", "uni_modules/uview-ultra/components/up-calendar/month.uvue", 330, 11), } as UTSJSONObject
				if (this.color != '#3c9cff') {
			...
			getMonthRect() {
				...
				const promiseAllArr: Array<Promise<NodeInfo>> = this.months.map((item: CalendarMonthItem, index):Promise<NodeInfo> => {
