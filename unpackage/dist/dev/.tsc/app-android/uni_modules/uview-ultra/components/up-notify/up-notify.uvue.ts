...
const config = {__$originalPosition: new UTSSourceMapPosition("config", "uni_modules/uview-ultra/components/up-notify/up-notify.uvue", 50, 7),
  top: 0,
  type: 'primary',
  color: '#ffffff',
  bgColor: '',
...
const backgroundColor = computed<UTSJSONObject>(() => {
  const style = { __$originalPosition: new UTSSourceMapPosition("style", "uni_modules/uview-ultra/components/up-notify/up-notify.uvue", 107, 9), } as UTSJSONObject
...
const _component_up_transition = resolveEasyComponent("up-transition",_easycom_up_transition)
  ...
  return _cV(_component_up_transition, _uM({
    ...
    default: withSlotCtx((): any[] => [
      _cE("view", _uM({
        class: _nC(["up-notify", [`up-notify--${tmpConfig.value['type']}`]]),
        style: _nS(notifyStyle.value)
      }), [
        isTrue(tmpConfig.value['safeAreaInsetTop'])
          ? _cV(_component_up_status_bar, _uM({ key: 0 }))
          : _cC("v-if", true),
        _cE("view", _uM({ class: "up-notify__warpper" }), [
          renderSlot(_ctx.$slots, "icon", {}, (): any[] => [
            isTrue(isIconVisible.value)
          ...
          _cE("text", _uM({
            ...
            style: _nS(notifyTextStyle.value)
          }), _tD(tmpConfig.value['message']), 5 /* TEXT, STYLE */)
        ])
      ], 6 /* CLASS, STYLE */)
    ]),
    _: 3 /* FORWARDED */
