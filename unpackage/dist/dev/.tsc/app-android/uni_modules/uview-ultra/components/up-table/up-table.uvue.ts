...
name: 'up-table',
  props: {
		border: {
			type: Boolean,
			default: defProps.getBoolean('table.border') ?? true
		},
		borderColor: {
			type: String,
...
const _cache = __ins.renderCache;
	...
	const tableStyle = computed(() => {
		let style = {__$originalPosition: new UTSSourceMapPosition("style", "uni_modules/uview-ultra/components/up-table/up-table.uvue", 66, 7),}
		if (props.border) {
	...
	// 监听border、align和borderColor的变化
	watch((): boolean => props.border, () => {
		change()
	})
