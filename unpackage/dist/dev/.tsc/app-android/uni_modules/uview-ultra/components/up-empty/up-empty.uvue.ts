...
const emptyStyle = computed(() => {
	const style = {__$originalPosition: new UTSSourceMapPosition("style", "uni_modules/uview-ultra/components/up-empty/up-empty.uvue", 141, 8),}
	style['marginTop'] = addUnit(props.marginTop)
	// 合并customStyle样式
	return style
})

const mergedEmptyStyle = computed((): UTSJSONObject => {
	const custom = addStyle(props.customStyle) as UTSJSONObject
	const empty = emptyStyle.value as UTSJSONObject
	return deepMerge(empty, custom) as UTSJSONObject
})

const imageDimensionStyle = computed((): UTSJSONObject => {
	return {
...
const textStyle = computed(() => {
	const style = {__$originalPosition: new UTSSourceMapPosition("style", "uni_modules/uview-ultra/components/up-empty/up-empty.uvue", 162, 8),}
...
const _component_up_icon = resolveEasyComponent("up-icon",_easycom_up_icon)
  ...
  return isTrue(_ctx.show)
    ? _cE("view", _uM({
        key: 0,
        class: "up-empty",
        style: _nS(mergedEmptyStyle.value)
      }), [
        isTrue(!isSrc.value)
          ...
          : _cE("image", _uM({
              key: 1,
              style: _nS(imageDimensionStyle.value),
              src: _ctx.icon,
              mode: "widthFix"
