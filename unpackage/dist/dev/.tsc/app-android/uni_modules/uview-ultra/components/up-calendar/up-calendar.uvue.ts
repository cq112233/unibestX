...
const __sfc__ = defineComponent({
	...
	data() {
		return {
			...
			inited: false,
			// 需要显示的月份的数组
			months: [] as Array<CalendarMonthItem>,
	...
	computed: {
		...
		currentMonths(): Array<CalendarMonthItem> {
			// @ts-ignore
	...
	methods: {
		...
		init() {
			...
			if (
				...
				// @ts-ignore
				console.log(this.innerMaxDate, " at uni_modules/uview-ultra/components/up-calendar/up-calendar.uvue:570")
		...
		setMonth() {
			...
			// 先清空数组
			// @ts-ignore
			this.months = [] as Array<CalendarMonthItem>
