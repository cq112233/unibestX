...
__ins.emit(event, ...do_not_transform_spread)
	...
	const iconWrapStyle = computed((): any => {
		// checkbox的整体样式
		const style = {__$originalPosition: new UTSSourceMapPosition("style", "uni_modules/uview-ultra/components/up-checkbox/up-checkbox.uvue", 263, 9),};
	...
	const checkboxStyle = computed((): any => {
		const style = {__$originalPosition: new UTSSourceMapPosition("style", "uni_modules/uview-ultra/components/up-checkbox/up-checkbox.uvue", 286, 9),};
...
const _component_up_icon = resolveEasyComponent("up-icon",_easycom_up_icon)
  ...
  return _cE("view", _uM({
    class: _nC(["up-checkbox cursor-pointer", [`up-checkbox-label--` + (unref(parentData)['iconPlacement'] != null ? unref(parentData)['iconPlacement'].toString() : ''),
			(unref(parentData)['borderBottom'] != false && unref(parentData)['placement'] != null
				&& unref(parentData)['placement'].toString() == 'column')
				? 'up-border-bottom' : '']]),
