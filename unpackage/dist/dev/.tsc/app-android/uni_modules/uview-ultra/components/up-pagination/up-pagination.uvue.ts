...
const _component_up_icon = resolveEasyComponent("up-icon",_easycom_up_icon)
  ...
  return isTrue(shouldShow.value)
    ? _cE("view", _uM({
        ...
        _cE("view", _uM({
          class: _nC(["up-pagination-btn", _uM({ 'btn-disabled': _ctx.currentPage <= 1 })]),
        ...
        isTrue(showPager.value)
          ? _cE(Fragment, _uM({ key: 0 }), RenderHelpers.renderList(displayedPages.value, (page, index, __index, _cached): any => {
              return _cE("view", _uM({
                key: index,
                class: _nC(["up-pagination-item", _uM({ 'item-active': page == currentPageStr.value })]),
                onClick: () => {goTo(page)}
              }), [
                _cE("text", _uM({
                  class: _nC(["up-pagination-text", _uM({ 'text-active': page == currentPageStr.value })])
        ...
        isTrue(showTotal.value)
          ? _cE("view", _uM({
              ...
              _cE("text", _uM({ class: "up-pagination-text" }), "共 " + _tD(_ctx.total) + " 条", 1 /* TEXT */)
            ])
          : _cC("v-if", true),
        _cE("view", _uM({
          class: _nC(["up-pagination-btn", _uM({ 'btn-disabled': _ctx.currentPage >= totalPages.value })]),
          style: _nS(_uM({ backgroundColor: _ctx.buttonBgColor, borderColor: _ctx.buttonBorderColor })),
          onClick: next
        }), [
          _ctx.nextText != ''
