...
import type { PropType } from 'vue'
	...
	type UPIndexListChildrenItemType = { __$originalPosition?: UTSSourceMapPosition<"UPIndexListChildrenItemType", "uni_modules/uview-ultra/components/up-index-list/up-index-list.uvue", 80, 7>;
		height: number,
		top: number
	}
  ...
  props: {
		...
		indexList: {
			...
			default: () : Array<any> => {
				...
				defProps.getArray<string>('indexList.indexList')!!.forEach((item: string) => {
					list.push(item)
				})
			    return list
			}
...
const _cache = __ins.renderCache;
	...
	const uIndexList = computed((): Array<any> => {
		if(props.indexList.length > 0) {
			return props.indexList as Array<any>
		} else {
			const list = [] as Array<any>
			indexList().forEach((item: string) => {
				list.push(item)
			})
			return list
		}
	...
	const indicatorTransitionStyle = computed((): UTSJSONObject => {
		return {
			position: 'absolute',
			right: '30px',
			top: addUnit(indicatorTop.value, 'px'),
			width: addUnit(indicatorHeight.value + 10),
			zIndex: 3
		} as UTSJSONObject
	})

	const indicatorBoxStyle = computed((): UTSJSONObject => {
		return {
			height: addUnit(indicatorHeight.value),
			width: addUnit(indicatorHeight.value)
		} as UTSJSONObject
	})

	function getItemTextStyle(index: number): UTSJSONObject {
		return {
	...
	// 监听字母索引的变化，重新设置尺寸
	watch((): Array<any> => uIndexList.value, () => {
		sleep(30).then(() => {
	...
	const keyVal = (item: any): string => {
		...
		const keyNames = ['key', 'name', 'value', 'label', 'text'] as string[]
	...
	const init = () => {
		...
		getIndexListRect().then((sizeScroll: NodeInfo) => {
			const h = sizeScroll?.height != null && sizeScroll.height! > 0 ? sizeScroll.height! : sys.value!!.windowHeight - parseInt(customNavHeight)
	...
	const setValueForTouch = async (currentIndex: number) => {
		...
		if (currentIndex !== activeIndex.value) {
			...
			console.log(scrollIntoView.value, " at uni_modules/uview-ultra/components/up-index-list/up-index-list.uvue:340")
			...
			let childrenItems = children.value.map((item: ComponentPublicInstance, index: number) => {
				...
				const child = {__$originalPosition: new UTSSourceMapPosition("child", "uni_modules/uview-ultra/components/up-index-list/up-index-list.uvue", 357, 11),
					height: childHeight,
					top: top
				}
				// 进行累加，给下一个item提供计算依据
	...
	const getItemStyle = (index: number): UTSJSONObject => {
		const style = { __$originalPosition: new UTSSourceMapPosition("style", "uni_modules/uview-ultra/components/up-index-list/up-index-list.uvue", 528, 9), } as UTSJSONObject
...
const _component_up_transition = resolveEasyComponent("up-transition",_easycom_up_transition)
  ...
  return _cE("view", _uM({
    ...
    isTrue(_ctx.sticky && activeIndex.value >= 0 && activeIndex.value < uIndexList.value.length)
      ? _cE("view", _uM({
          key: 0,
          class: "up-index-list__sticky-header",
          style: _nS(_uM({"position":"absolute","top":"0","left":"0","right":"0","z-index":"2"}))
        }), [
          _cV(_component_up_index_anchor, _uM({
            text: keyVal(uIndexList.value[activeIndex.value])
          }), null, 8 /* PROPS */, ["text"])
        ], 4 /* STYLE */)
      : _cC("v-if", true),
    _cE("scroll-view", _uM({
    ...
    _cE("view", _uM({
      ...
      style: _nS(letterStyle.value),
      onTouchstart: touchStart,
      onTouchmove: touchMove,
      onTouchend: touchEnd,
      onTouchcancel: touchEnd
    }), [
      _cE(Fragment, null, RenderHelpers.renderList(uIndexList.value, (item, index, __index, _cached): any => {
        return _cE("view", _uM({
          class: _nC(["up-index-list__letter__item", _uM({'up-index-list__letter__item--active': activeIndex.value == index && _ctx.activeColor == '#5677fc'})]),
          key: index,
          style: _nS(getItemStyle(index))
        }), [
          _cE("text", _uM({
            class: "up-index-list__letter__item__index",
            style: _nS(getItemTextStyle(index))
          }), _tD(keyVal(item)), 5 /* TEXT, STYLE */)
        ], 6 /* CLASS, STYLE */)
      }), 128 /* KEYED_FRAGMENT */)
    ...
    _cV(_component_up_transition, _uM({
      ...
      default: withSlotCtx((): any[] => [
        _cE("view", _uM({
          class: _nC(["up-index-list__indicator", ['up-index-list__indicator--show']]),
          style: _nS(indicatorBoxStyle.value)
        }), [
          _cE("text", _uM({ class: "up-index-list__indicator__text" }), _tD(activeIndex.value >= 0 ? keyVal(uIndexList.value[activeIndex.value]) : ''), 1 /* TEXT */)
        ], 4 /* STYLE */)
      ]),
      _: 1 /* STABLE */
