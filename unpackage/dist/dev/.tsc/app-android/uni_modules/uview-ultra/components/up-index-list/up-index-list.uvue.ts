...
import type { PropType } from 'vue'
	...
	type UPIndexListChildrenItemType = { __$originalPosition?: UTSSourceMapPosition<"UPIndexListChildrenItemType", "uni_modules/uview-ultra/components/up-index-list/up-index-list.uvue", 90, 7>;
		height: number,
		top: number
	}
...
const _cache = __ins.renderCache;
	...
	const setValueForTouch = async (currentIndex: number) => {
		...
		if (currentIndex !== activeIndex.value) {
			...
			console.log(scrollIntoView.value, " at uni_modules/uview-ultra/components/up-index-list/up-index-list.uvue:319")
			...
			let childrenItems = children.value.map((item: ComponentPublicInstance, index: number) => {
				...
				const child = {__$originalPosition: new UTSSourceMapPosition("child", "uni_modules/uview-ultra/components/up-index-list/up-index-list.uvue", 336, 11),
					height: childHeight,
					top: top
				}
				// 进行累加，给下一个item提供计算依据
	...
	const getItemStyle = (index: number): UTSJSONObject => {
		const style = { __$originalPosition: new UTSSourceMapPosition("style", "uni_modules/uview-ultra/components/up-index-list/up-index-list.uvue", 507, 9), } as UTSJSONObject
...
const _component_up_transition = resolveEasyComponent("up-transition",_easycom_up_transition)
  ...
  return _cE("view", _uM({
    ...
    _cE("view", _uM({
      ...
      style: _nS(_uM({top: _ctx.$up.addUnit(letterInfo.value['top'] ?? '') , transform: 'translateY(-50%)'})),
      ...
      _cE(Fragment, null, RenderHelpers.renderList(uIndexList.value, (item, index, __index, _cached): any => {
        return _cE("view", _uM({
          ...
          _cE("text", _uM({
            ...
            style: _nS(_uM({color: activeIndex.value === index ? '#fff' : _ctx.inactiveColor}))
    ...
    _cV(_component_up_transition, _uM({
      ...
      default: withSlotCtx((): any[] => [
        _cE("view", _uM({
          ...
          style: _nS(_uM({
					height: _ctx.$up.addUnit(indicatorHeight.value),
					width: _ctx.$up.addUnit(indicatorHeight.value)
				}))
        }), [
