...
import type { PropType } from 'vue'
	...
	type UPIndexListChildrenItemType = { __$originalPosition?: UTSSourceMapPosition<"UPIndexListChildrenItemType", "uni_modules/uview-ultra/components/up-index-list/up-index-list.uvue", 82, 7>;
		height: number,
		top: number
	}
...
const _cache = __ins.renderCache;
	...
	const setValueForTouch = async (currentIndex: number) => {
		...
		if (currentIndex !== activeIndex.value) {
			...
			let childrenItems = children.value.map((item: ComponentPublicInstance, index: number) => {
				...
				const child = {__$originalPosition: new UTSSourceMapPosition("child", "uni_modules/uview-ultra/components/up-index-list/up-index-list.uvue", 328, 11),
					height: childHeight,
					top: top
				}
				// 进行累加，给下一个item提供计算依据
	...
	const getItemStyle = (index: number): UTSJSONObject => {
		const style = { __$originalPosition: new UTSSourceMapPosition("style", "uni_modules/uview-ultra/components/up-index-list/up-index-list.uvue", 499, 9), } as UTSJSONObject
