...
function getColumnStyle(index: number): UTSJSONObject {
    const style = { __$originalPosition: new UTSSourceMapPosition("style", "uni_modules/uview-ultra/components/up-waterfall/up-waterfall.uvue", 89, 11), } as UTSJSONObject
    if (index > 0) {
...
function remove(id: any | null) {
    ...
    if (idx != -1) {
        const newData: UTSJSONObject[] = UTSAndroid.consoleDebugError(JSON.parse(JSON.stringify(dataList)), " at uni_modules/uview-ultra/components/up-waterfall/up-waterfall.uvue:108") as UTSJSONObject[]
...
function modify(id: any, key: string, val: any) {
    ...
    if (idx != -1) {
        const newData: UTSJSONObject[] = UTSAndroid.consoleDebugError(JSON.parse(JSON.stringify(dataList)), " at uni_modules/uview-ultra/components/up-waterfall/up-waterfall.uvue:121") as UTSJSONObject[]
        newData[idx][key] = val
        emit('update:modelValue', newData)
        emit('update:value', newData)
    }
