...
export async function registerFontIcon(config : FontIconConfig) : Promise<void> {
	...
	if (Object.keys(icons).length > 0) {
		...
		__f__('log','at uni_modules/lime-icon/index.ts:327',`已注册字体图标库: ${prefix} (内置${Object.keys(icons).length}个图标)`)
...
export async function registerIconify(config : IconifyConfig) : Promise<void> {
	...
	if (Object.keys(icons).length > 0) {
		...
		iconData.set(prefix, icons)
		iconDataChangeCount.value++ // 增加计数器以触发响应式更新
		__f__('log','at uni_modules/lime-icon/index.ts:398',`已注册本地Iconify图标库: ${prefix} (内置${Object.keys(icons).length}个图标)`)
		...
		try {
			...
			iconData.set(prefix, jsonIcons)
			iconDataChangeCount.value++ // 增加计数器以触发响应式更新
			__f__('log','at uni_modules/lime-icon/index.ts:410',`已注册本地Iconify图标库: ${prefix} (从JSON加载${Object.keys(jsonIcons).length}个图标)`)
		} catch (error) {
