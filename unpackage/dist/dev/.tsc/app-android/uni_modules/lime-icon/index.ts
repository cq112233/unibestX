...
export function setIconifyApi(apiUrl: string): void {
	...
	__f__('log','at uni_modules/lime-icon/index.ts:154',`已设置默认Iconify API: ${DEFAULT_ICONIFY_API}`)
}
...
export async function registerFontIcon(config : FontIconConfig) : Promise<void> {
	...
	if (!isNullish(config.fontUrl)) {
		...
		uni.loadFontFace({
			...
			success: () => {
				...
				__f__('log','at uni_modules/lime-icon/index.ts:317',`字体加载成功: ${config.fontFamily}; 正在加载字体数量: ${loadingFonts.value.length}`)
			},
			fail: (err) => {__f__('error','at uni_modules/lime-icon/index.ts:319',`字体加载失败: ${config.fontFamily}`, err)}
		})
	}
	...
	if (UTSJSONObject.keys(icons).length > 0) {
		...
		__f__('log','at uni_modules/lime-icon/index.ts:327',`已注册字体图标库: ${prefix} (内置${UTSJSONObject.keys(icons).length}个图标)`)
	...
	// 从JSON加载
		...
		try {
			...
			iconDataChangeCount.value++ // 增加计数器以触发响应式更新
			__f__('log','at uni_modules/lime-icon/index.ts:340',`已注册字体图标库: ${prefix} (从JSON加载${Object.keys(jsonIcons).length}个图标)`)
		} catch (error) {
			__f__('log','at uni_modules/lime-icon/index.ts:342','jsonUrl', jsonUrl)
			__f__('error','at uni_modules/lime-icon/index.ts:343',`注册字体图标库失败: ${prefix}`, error)
			throw error
		}
	} else {
		__f__('warn','at uni_modules/lime-icon/index.ts:347',`注册字体图标库: ${prefix}，但未提供图标数据`)
	}
}
...
async function loadAndMergeJson(prefix : string, jsonUrl : string) : Promise<void> {
	try {
		...
		// 打印合并成功信息
		__f__('log','at uni_modules/lime-icon/index.ts:374',`已合并图标库: ${prefix}，现有${Object.keys(mergedIcons).length}个图标`)
	} catch (error) {
		// 加载失败时打印警告，保留现有图标数据
		__f__('warn','at uni_modules/lime-icon/index.ts:377',`加载图标JSON失败: ${jsonUrl}，使用现有图标`)
	}
}
...
export async function registerIconify(config : IconifyConfig) : Promise<void> {
	...
	if (UTSJSONObject.keys(icons).length > 0) {
		...
		iconData.set(prefix, icons)
		iconDataChangeCount.value++ // 增加计数器以触发响应式更新
		__f__('log','at uni_modules/lime-icon/index.ts:398',`已注册本地Iconify图标库: ${prefix} (内置${UTSJSONObject.keys(icons).length}个图标)`)
		...
		try {
			...
			iconData.set(prefix, jsonIcons)
			iconDataChangeCount.value++ // 增加计数器以触发响应式更新
			__f__('log','at uni_modules/lime-icon/index.ts:410',`已注册本地Iconify图标库: ${prefix} (从JSON加载${UTSJSONObject.keys(jsonIcons).length}个图标)`)
		} catch (error) {
			__f__('error','at uni_modules/lime-icon/index.ts:412',`注册本地Iconify图标库失败: ${prefix}`, error)
			throw error
		}
	} else {
		// 远程Iconify API模式 - 不存储图标数据，使用时从API获取
		__f__('log','at uni_modules/lime-icon/index.ts:417',`已注册远程Iconify图标库: ${prefix} (API: ${apiUrl})`)
	}
}
