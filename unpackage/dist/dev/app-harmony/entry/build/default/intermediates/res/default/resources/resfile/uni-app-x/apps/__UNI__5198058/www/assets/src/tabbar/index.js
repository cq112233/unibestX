  ...
  setup(__props) {
    ...
    onMounted(() => {
      ...
      uni.hideTabBar(new UTSJSONObject({
        animation: false,
        fail: (err = null) => {
          uni.__f__("log", "at src/tabbar/index.uvue:60", "hideTabBar fail: ", err);
        }
      }));
    });
    return () => {
      ...
      _renderSharedDataEffect(() => {
        const _TABBAR_HEIGHT = TABBAR_HEIGHT;
        const _safeAreaBottom = _unref(safeAreaBottom);
        const _themeTokens = _unref(themeTokens);
        _setSharedDataStyle(__sharedData, 1, { height: `${_TABBAR_HEIGHT + _safeAreaBottom}px` });
        _setSharedDataStyle(__sharedData, 2, { height: `${TABBAR_CONTAINER_HEIGHT + _safeAreaBottom}px` });
        _setSharedDataStyle(__sharedData, 3, { height: `${_TABBAR_HEIGHT + _safeAreaBottom}px`, backgroundColor: _themeTokens.tabBg, borderTopColor: _themeTokens.tabBorder });
      });
      _createSharedDataFor(_setSharedDataScoped(__sharedData, 0, _createSharedDataVFor(__sharedDataScope, () => {
        ...
        const n3 = _createSharedDataComponent(TabbarItem, "10d18aee-" + _for_key0.value, {
          item: () => {
      ...
      _renderSharedDataEffect(() => {
        return _setSharedDataStyle(__sharedData, 4, { height: `${_unref(safeAreaBottom)}px`, backgroundColor: _unref(themeTokens).tabBg });
      });
      return __sharedData;
    };
  }
