    ...
    default: _withCtx(() => [
      _createElementVNode("view", { class: "rounded-12px p-16px mb-16px border-width-1px border-style-solid border-color-__e2e8f0_" }, [
        _createElementVNode("view", { class: "flex-row justify-between items-center mb-8px" }, [
          _createElementVNode("text", { class: "text-14px text-__64748b_" }, "当前登录状态:"),
          _createElementVNode(
            "text",
            {
        ...
        _createElementVNode("text", { class: "text-12px text-__94a3b8_ leading-normal" }, " 测试场景说明：Test 页面属于需登录拦截页面。未登录时点击跳转将拦截并重定向到登录页，登录成功后会自动跳回 Test 页并解析 query 传参。 ")
      ]),
      _createElementVNode("view", { class: "flex-row justify-between" }, [
        _createElementVNode("view", {
          ...
          _createElementVNode("text", { class: "text-__ffffff_ text-14px" }, "跳转 Test 页 (传参)")
        ]),
        _createElementVNode("view", {
          class: "flex-1 ml-12px bg-__ef4444_ rounded-8px h-42px flex flex-row items-center justify-center",
          onClick: $setup.handleLogout
        }, [
          _createElementVNode("text", { class: "text-__ffffff_ text-14px" }, "清除登录状态")
        ])
      ])
    ]),
    _: 1
