    ...
    checkSticky(scrollTop) {
      ...
      this.upGetRect("#" + this.elId).then((res) => {
        if (res != null && res.top != null) {
          const currentTop = res.top;
          if (this.initialTop <= 0 && currentTop > 0 && !this.fixed) {
            this.initialTop = currentTop + scrollTop;
          }
          const threshold = this.stickyTop;
          const shouldFixed = currentTop <= threshold || this.initialTop > 0 && scrollTop >= this.initialTop - threshold;
