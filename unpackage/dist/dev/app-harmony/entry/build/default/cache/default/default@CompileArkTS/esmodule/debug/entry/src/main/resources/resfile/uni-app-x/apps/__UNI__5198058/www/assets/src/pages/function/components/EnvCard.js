...
function _sfc_render(_ctx, _cache, $props, $setup, $data, $options) {
  return openBlock(), createBlock($setup["Card"], { title: "环境变量" }, {
    default: withCtx(() => [
      createCommentVNode(" 当前环境信息 "),
      createElementVNode("view", { class: "rounded-12px p-16px mb-16px border-width-1px border-style-solid border-color-__e2e8f0_" }, [
        createElementVNode("view", { class: "flex-row justify-between mb-10px" }, [
          createElementVNode("text", { class: "text-14px text-__718096_" }, "编译环境"),
          createElementVNode(
            "text",
            { class: "text-14px font-bold text-__2d3748_" },
            toDisplayString($setup.systemEnvLabel),
            1
            /* TEXT */
          )
        ]),
        createElementVNode("view", { class: "flex-row justify-between mb-10px" }, [
          createElementVNode("text", { class: "text-14px text-__718096_" }, "当前环境"),
          createElementVNode(
            "text",
            {
              class: "text-14px font-bold",
              style: normalizeStyle({ color: $setup.envColor })
            },
            toDisplayString($setup.currentEnvLabel),
            5
            /* TEXT, STYLE */
          )
        ]),
        createElementVNode("view", { class: "flex-row justify-between mb-10px" }, [
          createElementVNode("text", { class: "text-14px text-__718096_" }, "接口地址"),
          createElementVNode(
            "text",
            { class: "text-12px text-__64748b_ flex-1 ml-8px text-right" },
            toDisplayString($setup.envConfig.baseURL),
            1
            /* TEXT */
          )
        ]),
        createElementVNode("view", { class: "flex-row justify-between" }, [
          createElementVNode("text", { class: "text-14px text-__718096_" }, "是否已覆盖"),
          createElementVNode(
            "text",
            { class: "text-14px font-bold text-__2d3748_" },
            toDisplayString($setup.isOverridden ? "是" : "否"),
            1
            /* TEXT */
          )
        ])
      ]),
      createCommentVNode(" 环境切换 "),
      createElementVNode("view", { class: "rounded-12px p-16px mb-16px border-width-1px border-style-solid border-color-__e2e8f0_" }, [
      ...
      createElementVNode("view", {
        ...
        createElementVNode("text", { class: "text-__ffffff_ text-14px font-bold" }, "重置为系统默认")
      ])
    ]),
    _: 1
    /* STABLE */
