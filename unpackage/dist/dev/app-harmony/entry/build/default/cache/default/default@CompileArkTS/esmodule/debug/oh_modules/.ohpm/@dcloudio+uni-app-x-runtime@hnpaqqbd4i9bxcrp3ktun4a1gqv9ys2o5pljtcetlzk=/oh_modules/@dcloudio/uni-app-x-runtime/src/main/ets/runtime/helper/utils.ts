import { getCurrentPages } from "@normalized:N&&&@dcloudio/uni-runtime-harmony/helper/global&1.0.0";
...
export function cropPixelMap(pixmap: image.PixelMap, x: number, y: number, width: number | null, height: number | null) {
...
export function toRecord(data: unknown): any {
    const baseTypes = ['string', 'number', 'boolean'];
    let res: any = {};
    if (data instanceof Map) {
        data.forEach((value, key) => {
            res[key] = toRecord(value) as any;
        });
    }
    else if (Array.isArray(data)) {
        res = data.map(toRecord) as any;
    }
    else if (data === null ||
        data === undefined ||
        baseTypes.includes(typeof data)) {
        res = data as any;
    }
    else if (typeof data === 'object') {
        for (const key in data) {
            res[key] = toRecord((data as any)[key]) as any;
        }
    }
    else {
        res = undefined;
