    ...
    onReceive: (domain: string, appEventGroups: Array<hiAppEvent.AppEventGroup>) => {
        for (const eventGroup of appEventGroups) {
            for (const eventInfo of eventGroup.appEventInfos) {
                ...
                const timeValue = params['time'] as number || null;
                const time = timeValue == null ? Date.now().toString() : timeValue.toString();
                const external_log: Array<string> = params['external_log'];
                if (!Array.isArray(external_log) || external_log.length === 0 || typeof external_log[0] !== 'string')
                    continue;
                let crash_type = params['crash_type'] as string;
                if (crash_type === 'NativeCrash') {
                    crash_type = 'cppcrash';
                }
                else {
                    crash_type = 'jscrash';
                ...
                try {
                    ...
                    const saveDirPath = `${cachePath}/uni-crash`;
                    let canAccess = false;
                    try {
                    ...
                    savePath = `${saveDirPath}/${crash_type}-${new fileUri.FileUri(fileUri.getUriFromPath(filePath)).name}`;
...
const __deleteAppCrashInfo: DeleteAppCrashInfo = defineSyncApi('__deleteAppCrashInfo', (id: string | null = null) => {
    ...
    if (typeof id === 'string') {
        ...
        arr.forEach((item) => {
            try {
                fs.unlink(item.file).catch((e: any) => { });
            }
            catch (e) { }
        });
        setAppCrashInfo([]);
