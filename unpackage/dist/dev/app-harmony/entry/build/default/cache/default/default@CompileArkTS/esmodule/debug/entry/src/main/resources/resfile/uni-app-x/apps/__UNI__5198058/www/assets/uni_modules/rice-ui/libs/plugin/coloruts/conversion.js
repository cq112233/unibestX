import { p as presetColors } from "@normalized:N&&&entry/src/main/resources/resfile/uni-app-x/apps/__UNI__5198058/www/assets/uni_modules/rice-ui/libs/plugin/coloruts/constant&";
...
import { l as limitRange, s as splitColorStr, p as parseHSVorHSL } from "@normalized:N&&&entry/src/main/resources/resfile/uni-app-x/apps/__UNI__5198058/www/assets/uni_modules/rice-ui/libs/plugin/coloruts/utils&";
class Coloruts {
  constructor(input) {
    this.isValid = true;
    this.r = 0;
    ...
    if (typeof input == "string") {
    ...
    } else if (UTS.isInstanceOf(input, Coloruts)) {
      this.r = input.r;
      this.g = input.g;
      this.b = input.b;
      this.a = input.a;
      ...
      const data = input;
      this.r = limitRange(data["r"]);
      this.g = limitRange(data["g"]);
      this.b = limitRange(data["b"]);
      this.a = typeof data["a"] == "number" ? limitRange(data["a"], 1) : 1;
    } else if (isHsl()) {
      this.fromHsl(input);
    } else if (isHsv()) {
      this.fromHsv(input);
    } else {
      uni.__f__("warn", "at uni_modules/rice-ui/libs/plugin/coloruts/conversion.uts:78", `不支持当前的颜色值。${input}`);
    }
  }
  ...
  fromHslString(trimStr) {
    ...
    this.fromHsl(new UTSJSONObject({
      h: cells[0],
      s: cells[1],
      l: cells[2],
      a: cells[3]
  ...
  fromHsvString(trimStr) {
    const cells = splitColorStr(trimStr, parseHSVorHSL);
    this.fromHsv(new UTSJSONObject({
      h: cells[0],
      s: cells[1],
      v: cells[2],
      a: cells[3]
   ...
   */
  tint(amount = 10) {
    return this.mix(new UTSJSONObject({ r: 255, g: 255, b: 255, a: 1 }), amount);
  }
  /**
   * 将颜色与纯黑色混合，从0到100。
   ...
   */
  shade(amount = 10) {
    return this.mix(new UTSJSONObject({ r: 0, g: 0, b: 0, a: 1 }), amount);
  }
  /**
   * 将当前的颜色加深
   * amount 默认10
  ...
  darken(amount = 10) {
    ...
    return this._c(new UTSJSONObject({
      h,
      s,
      l,
      a: this.a
