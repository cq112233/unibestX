...
export class UniInputFileElementImpl extends UniFormControlElementImpl<string, FrameNode | TextareaFrameNode> implements IInputNode {
    ...
    protected updateSelectionOnFocus() {
        // 需要聚焦状态后设置光标位置
        setTimeout(() => {
            if (this.selection.end !== -1 && this.selection.end >= this.selection.start) {
                ...
                this.controller.setTextSelection(cursor, cursor);
            }
        }, 0);
    }
    protected getCursorPosition(): number {
        return this.cursor >= 0 ? this.cursor : this.value.length;
    }
    protected handleTextSelectionChange(_selectionStart: number, selectionEnd: number) {
    ...
    protected dispatchBlur() {
        if (this.needDispatchBLur) {
            this.dispatchEvent(new UniCustomEvent('blur', new UniCustomEventOptions<InputEventDetail>({ value: this.textValue, cursor: this.getCursorPosition() })));
        }
    }
    ...
    protected dispatchInput() {
        if (this.needDispatchInput) {
            this.dispatchEvent(new UniCustomEvent('input', new UniCustomEventOptions<InputEventDetail>({ value: this.textValue, cursor: this.getCursorPosition() })));
        }
    }
