...
import { parseStyleDecl } from "@normalized:N&&&@dcloudio/uni-runtime-harmony/helper/parser&1.0.0";
import type { CSSStyleDeclaration as ICSSStyleDeclaration } from '@dcloudio/uni-app-x/types/native';
...
import { isPropertySupportedByCSSOM, getCSSOMProperty } from "@normalized:N&&&@dcloudio/uni-runtime-harmony/dom/cssom/utils&1.0.0";
...
export function parseVar(value: string) {
    ...
    while (searchPosition < value.length) {
        const index = value.indexOf('var(', searchPosition);
        if (index < 0) {
            break;
        }
        let depth = 1;
        ...
        const subValue = value.substring(index + 4, nameEnd).trim();
        if (!/^--[^\s,()]+$/.test(subValue)) {
            searchPosition = index + 4;
            continue;
        }
        const fallback = comma < 0 ? '' : value.substring(comma + 1, end).trim();
        end++;
        const matchedValue = value.substring(index, end);
        if (index > position) {
            array.push({
