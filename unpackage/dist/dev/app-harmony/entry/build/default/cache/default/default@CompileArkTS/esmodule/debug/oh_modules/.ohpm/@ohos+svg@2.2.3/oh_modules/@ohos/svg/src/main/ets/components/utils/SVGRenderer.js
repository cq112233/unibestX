import { Box, Circle2, Colour, CurrentColor, Ellipse2, GradientElement, GraphicsElement, Group, Image2, Length, Line2, Mask, PaintReference, Path2, Pattern, Polygon2, PolyLine, Rect2, SolidColor, Svg, SvgElement, SvgElementBase, SvgLinearGradient, SvgRadialGradient, Switch, Symbol, Text2, TextContainer, TextPath, TextSequence, TRef, TSpan, Unit, Use, View } from '@normalized:N&&&@ohos/svg/src/main/ets/components/utils/SVGBase&2.2.3';
import Stack from '@ohos:util.Stack';
import { Isolation, LineCap, LineJoin, Style, TextAnchor, TextDirection, VectorEffect } from '@normalized:N&&&@ohos/svg/src/main/ets/components/utils/Style&2.2.3';
import { Paint } from '@normalized:N&&&@ohos/svg/src/main/ets/components/utils/mini_canvas&2.2.3';
import { CSSParser, RuleMatchContext, Source } from '@normalized:N&&&@ohos/svg/src/main/ets/components/utils/CSSParser&2.2.3';
import Matrix2D2 from '@normalized:N&&&@ohos/svg/src/main/ets/components/utils/Matrix&2.2.3';
import { Alignment, PreserveAspectRatio, Scale } from '@normalized:N&&&@ohos/svg/src/main/ets/components/PreserveAspectRatio&2.2.3';
import image from '@ohos:multimedia.image';
import util from '@ohos:util';
import intl from '@ohos:intl';
import { GlobalContext } from '@normalized:N&&&@ohos/svg/src/main/ets/components/GlobalContext&2.2.3';
import LogUtil from '@normalized:N&&&@ohos/svg/src/main/ets/components/LogUtil&2.2.3';
...
export class SVGRenderer {
    ...
    async renderImage(obj, context) {
        ...
        if (pic == null) {
            ...
            let pixelMap = null;
            let rawFileDescriptor = null;
            try {
                rawFileDescriptor = context.resourceManager.getRawFdSync(obj.href);
                imageSource = image.createImageSource(rawFileDescriptor.fd);
                ...
                // 释放资源
                this.releaseResource(imageSource, pixelMap, rawFileDescriptor?.fd);
            }
        }
        else {
            this.viewportFill();
            ...
            try {
                ...
                imageSource = image.createImageSource(array.buffer);
                pixelMap = await imageSource.createPixelMap();
                ...
                LogUtil.error(`base64 renderImage error: ${error}`);
                throw new Error("base64 renderImage error:" + error);
            }
            finally {
                // 释放资源
                this.releaseResource(imageSource, pixelMap, null);
            }
        }
    }
    /**
     ...
     * @param fd
     */
    releaseResource(imageSource, pixelMap, fileFd) {
        if (imageSource) {
            imageSource.release().catch((err) => {
                LogUtil.error(`is fd imageSource release error: ${err}`);
            });
        }
        if (pixelMap) {
            pixelMap.release().catch((err) => {
                LogUtil.error(`is fd pixelMap release error: ${err}`);
            });
        }
        if (fileFd) {
            fileIo.close(fileFd).catch((err) => {
                LogUtil.error(`is fd fileIo close error: ${err}`);
            });
        }
    }
