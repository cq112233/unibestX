...
import { IntegerParser } from "@normalized:N&&&@ohos/svg/src/main/ets/components/utils/IntegerParser&2.2.3";
import { AttribOp, Combinator, CSSParser, PseudoClassAnPlusB, PseudoClassEmpty, PseudoClassIdents, PseudoClassNot, PseudoClassNotSupported, PseudoClassOnlyChild, PseudoClassRoot, PseudoClassTarget, Selector, SimpleSelector } from "@normalized:N&&&@ohos/svg/src/main/ets/components/utils/CSSParser&2.2.3";
import type { PseudoClass } from "@normalized:N&&&@ohos/svg/src/main/ets/components/utils/CSSParser&2.2.3";
...
export class CSSTextScanner extends TextScanner {
    ...
    nextSimpleSelector(selector: Selector): boolean {
        ...
        while (!this.empty()) {
            if (this.consume('.')) {
                ...
                if (value == null) {
                    LogUtil.error("Invalid \".class\" simpleSelectors");
            ...
            if (this.consume('#')) {
                ...
                let value: string = this.nextIdentifier();
                if (value == null) {
                    LogUtil.error("Invalid \"#id\" simpleSelectors");
            ...
            if (this.consume('[')) {
                ...
                if (attrName == null) {
                    LogUtil.error("Invalid attribute simpleSelectors");
    ...
    private nextAnPlusB(): AnPlusB {
        ...
        if (this.consume("odd"))
            ...
            let aSign: number = 1, bSign = 1;
            if (this.consume('+')) {
            }
            else if (this.consume('-')) {
                bSign = -1;
            }
            let a: IntegerParser = null, b = IntegerParser.parseInt(this.input, this.position, this.inputLength, false);
            if (b != null)
            ...
            result = new AnPlusB((a == null) ? 0 : aSign * a.getValue(), (b == null) ? 0 : bSign * b.getValue());
        }
        this.skipWhitespace();
        if (this.consume(')'))
            return result;
    ...
    private parsePseudoClass(selector: Selector, selectorPart: SimpleSelector) {
        let ident: string = this.nextIdentifier();
        if (ident == null) {
            LogUtil.error("Invalid pseudo class");
        ...
        switch (ident) {
            ...
            case PseudoClassIdents.nth_last_of_type:
                ...
                if (ab == null) {
                    LogUtil.error("Invalid or missing parameter section for pseudo class: " + ident);
            ...
            case PseudoClassIdents.indeterminate:
                ...
                break;
            default:
                LogUtil.error("Unsupported pseudo class: " + ident);
