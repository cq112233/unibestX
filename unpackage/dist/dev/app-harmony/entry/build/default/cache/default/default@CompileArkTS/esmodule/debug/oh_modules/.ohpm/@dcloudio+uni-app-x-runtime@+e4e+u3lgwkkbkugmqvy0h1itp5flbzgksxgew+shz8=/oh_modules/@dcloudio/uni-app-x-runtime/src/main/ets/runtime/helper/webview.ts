...
export function createDownloadDelegate() {
    const delegate: harmonyWebView.WebDownloadDelegate = new harmonyWebView.WebDownloadDelegate();
    delegate.onBeforeDownload((webDownloadItem: harmonyWebView.WebDownloadItem) => {
        ...
        documentPicker
            ...
            const downloadTmpPath = getEnv().TEMP_PATH +
                '/download/' +
                suggestedFileName +
                '.' +
                uniqueDownloadId++;
    ...
    delegate.onDownloadFinish(async (webDownloadItem) => {
        ...
        const toFile = await fs.open(downloadItemSaveUri, fs.OpenMode.CREATE | fs.OpenMode.WRITE_ONLY);
