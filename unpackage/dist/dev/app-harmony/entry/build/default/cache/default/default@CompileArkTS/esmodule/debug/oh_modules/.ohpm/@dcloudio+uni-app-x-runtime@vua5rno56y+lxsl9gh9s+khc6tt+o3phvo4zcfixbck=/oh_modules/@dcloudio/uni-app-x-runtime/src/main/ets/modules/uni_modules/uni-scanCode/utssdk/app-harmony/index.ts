import { UTSObject, defineAsyncApi, UTS, UTSHarmony } from "@normalized:N&&&@dcloudio/uni-app-framework/index&1.0.0";
...
class ScanCodeSuccess extends UTSObject {
    result!: string;
    scanType!: string;
}
class ScanCodeFail extends UTSObject {
}
...
class ScanCodeOptions extends UTSObject {
...
export { ScanCodeFail as ScanCodeFail, ScanCodeSuccess as ScanCodeSuccess, ScanCodeOptions as ScanCodeOptions };
const isScanCoreSupported = canIUse('SystemCapability.Multimedia.Scan.Core');
...
const HarmonyScanTypeMap = isScanCoreSupported ? new Map<UniScanOptionsTypes, scanCore.ScanType[]>([
    [
        'barCode',
        [
...
]) : new Map<UniScanOptionsTypes, scanCore.ScanType[]>([]);
...
const UniScanTypeMap = isScanCoreSupported ? new Map<HarmonyScanResultTypes, UniScanResultTypes>([
    [
...
]) : new Map<HarmonyScanResultTypes, UniScanResultTypes>([]);
const scanCode: ScanCode = defineAsyncApi<ScanCodeOptions, ScanCodeSuccess>(API_SCAN_CODE, (options: ScanCodeOptions, exec: ApiExecutor<ScanCodeSuccess>) => {
    ...
    if (options.scanType && Array.isArray(options.scanType) && options.scanType.length > 0) {
        for (let i = 0; i < options.scanType.length; i++) {
            ...
            const harmonyScanTypes = UTS.mapGet(HarmonyScanTypeMap, uniScanType);
            if (!harmonyScanTypes) {
                continue;
            }
    ...
    scanBarcode.startScanForResult(UTSHarmony.getUIAbilityContext()!, scanOptions, (err, data) => {
        ...
        exec.resolve({
            ...
            scanType: UTS.mapGet(UniScanTypeMap, data.scanType as HarmonyScanResultTypes) || ''
        } as ScanCodeSuccess);
    });
}) as ScanCode;
export type { ScanCode as ScanCode, ScanCodeCompleteCallback as ScanCodeCompleteCallback, ScanCodeFailCallback as ScanCodeFailCallback, ScanCodeSuccessCallback as ScanCodeSuccessCallback, ScanCodeSupportedTypes as ScanCodeSupportedTypes };
export { scanCode as scanCode };
