...
function tryExec(fn: ((...args: any[]) => void) | undefined, args: any[]) {
    if (!fn) {
        return;
    }
    try {
 ...
 * }
 */
export function requestSystemPermission(permissions: string[], success?: (allRight: boolean, grantedList: string[]) => void, fail?: (doNotAskAgain: boolean, grantedList: string[]) => void): void {
    const atm = abilityAccessCtrl.createAtManager();
    atm.requestPermissionsFromUser(getAbilityContext()!, permissions as Permissions[], (err, data) => {
        if (err) {
            fail && fail(false, []);
            return;
        }
