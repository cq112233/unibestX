...
export class SVGRenderer {
  ...
  private updateStyle(state: RendererState, style: Style) {
    ...
    if (this.isSpecified(style, Style.SPECIFIED_FONT_STYLE)) {
      ...
      state.fillPaint.setFontStyle(state.style.fontStyle as any);
      state.strokePaint.setFontStyle(state.style.fontStyle as any);
    }
  ...
  private async renderImage(obj: Image2,context?:common.UIAbilityContext) {
    ...
    if (pic == null) {
      ...
      let rawFileDescriptor: resourceManager.RawFileDescriptor | null = null;
   ...
   * @param fd
   */
  private releaseResource(imageSource: image.ImageSource | null, pixelMap: image.PixelMap | null,
    fileFd: number | null) {
    if (imageSource) {
