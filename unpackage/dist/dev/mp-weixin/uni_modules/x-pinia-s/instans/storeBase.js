...
const uni_modules_xPiniaS_instans_subscriptions = require("./subscriptions.js");
class PiniaStoreBase {
  constructor() {
    this.$id = "";
    this._state = null;
    this._stateSubs = new uni_modules_xPiniaS_instans_subscriptions.StateSubscriptionList();
    this._actionSubs = new uni_modules_xPiniaS_instans_subscriptions.ActionSubscriptionList();
  ...
  bindState(state = null) {
    if (this._bound) {
      common_vendor.index.__f__("warn", "at uni_modules/x-pinia-s/instans/storeBase.uts:106", "[x-pinia-s][" + this.$id + "] bindState 只能调用一次");
      return null;
    }
    this._bound = true;
    this._state = state;
  ...
  _invokeAction(name, fn, args) {
    const actionCtx = new uni_modules_xPiniaS_instans_types.ActionContext({
      name,
      storeId: this.$id,
      args
    });
    ...
    try {
      ...
      for (let i = 0; i < triggerResult.errorList.length; i++) {
        try {
          ...
          common_vendor.index.__f__("warn", "at uni_modules/x-pinia-s/instans/storeBase.uts:179", "[x-pinia-s][" + this.$id + "] onError callback error:", e2);
        }
      }
      throw e;
    }
    for (let i = 0; i < triggerResult.afterList.length; i++) {
      try {
        ...
        common_vendor.index.__f__("warn", "at uni_modules/x-pinia-s/instans/storeBase.uts:188", "[x-pinia-s][" + this.$id + "] after callback error:", e3);
      }
    }
    return result;
  }
  ...
  $patch(partial) {
    ...
    const m = new uni_modules_xPiniaS_instans_types.SubscriptionMutation({
      type: "patch object",
      storeId: this.$id,
      payload: partial,
      timestamp: Date.now()
  ...
  $dispose() {
    if (this._watchStopper != null) {
      try {
        ...
        common_vendor.index.__f__("warn", "at uni_modules/x-pinia-s/instans/storeBase.uts:244", "[x-pinia-s][" + this.$id + "] watch stop error:", e);
      }
      this._watchStopper = null;
    }
    ...
    if (this._scope != null) {
      try {
        ...
        common_vendor.index.__f__("warn", "at uni_modules/x-pinia-s/instans/storeBase.uts:255", "[x-pinia-s][" + this.$id + "] scope stop error:", e);
      }
      this._scope = null;
    }
    if (this._pinia != null) {
  ...
  _setupBy(id, pinia) {
    ...
    if (this._state != null) {
      ...
      const selfRef = this;
      this._watchStopper = common_vendor.watch(() => {
        return stateRef;
      }, () => {
        const m = new uni_modules_xPiniaS_instans_types.SubscriptionMutation({
          type: "direct",
...
exports.PiniaStoreBase = PiniaStoreBase;
//# sourceMappingURL=../../../../.sourcemap/mp-weixin/uni_modules/x-pinia-s/instans/storeBase.js.map
