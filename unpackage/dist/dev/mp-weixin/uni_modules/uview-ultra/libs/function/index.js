...
const uni_modules_uviewUltra_libs_function_test = require("./test.js");
require("./digit.js");
...
function bem(name, fixed, change) {
  ...
  const classes = new common_vendor.UTSJSONObject({});
  if (fixed.length > 0) {
    fixed.map((item) => {
  ...
  return common_vendor.UTSJSONObject.keys(classes).join(" ");
}
...
function getPx(valueOri = null, unit = false) {
  ...
  if (uni_modules_uviewUltra_libs_function_test.number(value)) {
...
function sleep(value = 30) {
  return new Promise((resolve) => {
    setTimeout(() => {
      resolve(new common_vendor.UTSJSONObject({}));
    }, value);
  });
}
function os() {
  return common_vendor.index.getSystemInfoSync().osName;
}
function sys() {
...
function guid(len = 32, firstU = true, radix = 0) {
  ...
  if (firstU) {
    common_vendor.UTS.arrayShift(uuid);
    return `u${uuid.join("")}`;
  }
  return uuid.join("");
}
...
function addUnit(pvalue = null, punit = "") {
  ...
  let unit = punit;
  if (unit == "" || unit == null) {
    unit = uni_modules_uviewUltra_libs_config_config.config["unit"];
  }
  let valueStr = value.toString();
  return uni_modules_uviewUltra_libs_function_test.number(valueStr) ? `${valueStr}${unit}` : valueStr;
}
function deepClone(obj) {
  return common_vendor.UTS.JSON.parse(common_vendor.UTS.JSON.stringify(obj));
}
function deepMerge(targetOrigin = new common_vendor.UTSJSONObject({}), source = new common_vendor.UTSJSONObject({})) {
  let targetJson = common_vendor.UTS.JSON.parse(common_vendor.UTS.JSON.stringify(targetOrigin));
  let sourceJson = common_vendor.UTS.JSON.parse(common_vendor.UTS.JSON.stringify(source));
  let target = new common_vendor.UTSJSONObject({});
  target = Object.assign(targetJson, sourceJson);
  return target;
}
function error(err) {
  {
    common_vendor.index.__f__("error", "at uni_modules/uview-ultra/libs/function/index.uts:357", `uview-plus提示：${err}`);
  }
}
...
function timeFormat(dateTime = 0, formatStr = "yyyy-mm-dd") {
  ...
  const timeSource = new common_vendor.UTSJSONObject({
...
function getProperty(obj = null, key) {
  ...
  return (_b = obj[key]) !== null && _b !== void 0 ? _b : new common_vendor.UTSJSONObject({});
}
function setNestedValue(obj = null, keys, index, value = null) {
  ...
  if (nextObj == null || typeof nextObj !== "object") {
    nextObj = new common_vendor.UTSJSONObject({});
    obj[k] = nextObj;
  }
...
exports.upGetRect = upGetRect;
//# sourceMappingURL=../../../../../.sourcemap/mp-weixin/uni_modules/uview-ultra/libs/function/index.js.map
