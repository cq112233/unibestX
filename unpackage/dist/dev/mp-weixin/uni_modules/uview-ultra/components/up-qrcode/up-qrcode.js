  ...
  methods: {
    makeCode() {
      if (isEmpty(this.val)) {
        ...
        this.cells = [];
        this.$emit("error", new common_vendor.UTSJSONObject({ message: this.error }));
        return null;
      }
      try {
        this.loading = true;
    ...
    preview(e) {
      this.$emit("preview", new common_vendor.UTSJSONObject({
        url: this.result
      }), e);
    },
    longpress() {
      this.toTempFilePath(new common_vendor.UTSJSONObject({
        success: (res) => {
