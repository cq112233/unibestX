"use strict";
const common_vendor = require("../../../../common/vendor.js");
const uni_modules_uviewUltra_libs_function_index = require("../../libs/function/index.js");
const _sfc_main = /* @__PURE__ */ common_vendor.defineComponent({
  __name: "up-lazy-load",
  props: {
    index: {
      type: [Number, String]
    },
    // 要显示的图片
    image: {
      type: String,
      default: ""
    },
    // 图片裁剪模式
    imgMode: {
      type: String,
      default: "widthFix"
    },
    // 占位图片路径
    loadingImg: {
      type: String,
      default: "data:image/png;base64,"
    },
    // 加载失败的错误占位图
    errorImg: {
      type: String,
      default: "data:image/png;base64,"
    },
    // 图片进入可见区域前多少像素时，单位rpx，开始加载图片
    // 负数为图片超出屏幕底部多少距离后触发懒加载，正数为图片顶部距离屏幕底部多少距离时触发(图片还没出现在屏幕上)
    threshold: {
      type: [Number, String],
      default: 100
    },
    // 淡入淡出动画的过渡时间
    duration: {
      type: [Number, String],
      default: 500
    },
    // 渡效果的速度曲线，各个之间差别不大，因为这是淡入淡出，且时间很短，不是那些变形或者移动的情况，会明显
    // linear|ease|ease-in|ease-out|ease-in-out|cubic-bezier(n,n,n,n);
    effect: {
      type: String,
      default: "ease-in-out"
    },
    // 是否使用过渡效果
    isEffect: {
      type: Boolean,
      default: true
    },
    // 圆角值
    borderRadius: {
      type: [Number, String],
      default: 0
    },
    // 图片高度，单位rpx
    height: {
      type: [Number, String],
      default: "200"
    }
  },
  emits: ["click", "load", "error"],
  setup(__props, _a) {
    var __emit = _a.emit;
    const props = __props;
    const emit = __emit;
    const elIndex = uni_modules_uviewUltra_libs_function_index.guid();
    const isShow = common_vendor.ref(false);
    const opacity = common_vendor.ref(1);
    const time = common_vendor.ref(props.duration);
    const loadStatus = common_vendor.ref("");
    const isError = common_vendor.ref(false);
    let contentObserver = null;
    const imgHeight = common_vendor.computed(() => {
      return uni_modules_uviewUltra_libs_function_index.addUnit(props.height);
    });
    const wrapStyle = common_vendor.computed(() => {
      const br = props.borderRadius != null ? props.borderRadius.toString() : "0";
      const t = parseFloat(time.value.toString());
      return new common_vendor.UTSJSONObject({
        opacity: opacity.value,
        borderRadius: br + "rpx",
        transition: "opacity " + (t / 1e3).toString() + "s ease-in-out"
      });
    });
    const imageStyle = common_vendor.computed(() => {
      const br = props.borderRadius != null ? props.borderRadius.toString() : "0";
      return new common_vendor.UTSJSONObject({
        borderRadius: br + "rpx",
        height: imgHeight.value
      });
    });
    const getThreshold = common_vendor.computed(() => {
      const thresholdValue = parseFloat(props.threshold.toString());
      const thresholdPx = common_vendor.index.rpx2px(Math.abs(thresholdValue));
      return thresholdValue < 0 ? -thresholdPx : thresholdPx;
    });
    common_vendor.watch(isShow, (nVal) => {
      if (props.isEffect == false)
        return null;
      time.value = 0;
      opacity.value = 0;
      setTimeout(() => {
        time.value = props.duration;
        opacity.value = 1;
      }, 30);
    });
    const imageValue = common_vendor.computed(() => {
      return props.image;
    });
    common_vendor.watch(imageValue, (n) => {
      if (n == "") {
        isError.value = true;
      } else {
        init();
        isError.value = false;
      }
    });
    function init() {
      isError.value = false;
      loadStatus.value = "";
    }
    function clickImg() {
      if (isShow.value == false)
        ;
      else if (isError.value == true)
        ;
      else
        ;
      emit("click", props.index);
    }
    function imgLoaded() {
      if (loadStatus.value == "") {
        loadStatus.value = "lazyed";
      } else if (loadStatus.value == "lazyed") {
        loadStatus.value = "loaded";
        emit("load", props.index);
      }
    }
    function errorImgLoaded() {
      emit("error", props.index);
    }
    function loadError() {
      isError.value = true;
    }
    function disconnectObserver() {
      if (contentObserver != null) {
        contentObserver.disconnect();
        contentObserver = null;
      }
    }
    common_vendor.onBeforeUnmount(() => {
      disconnectObserver();
    });
    common_vendor.onMounted(() => {
      common_vendor.nextTick$1(() => {
        common_vendor.index.$once("uOnReachBottom", (arg = null) => {
          if (isShow.value == false)
            isShow.value = true;
        });
      });
      setTimeout(() => {
        const observer = common_vendor.index.createIntersectionObserver(this);
        observer.relativeToViewport(new common_vendor.UTSJSONObject({
          bottom: getThreshold.value
        })).observe(".up-lazy-item-" + elIndex, (res = null) => {
          if (res.intersectionRatio > 0) {
            isShow.value = true;
            disconnectObserver();
          }
        });
        contentObserver = observer;
      }, 30);
    });
    return (_ctx, _cache) => {
      "raw js";
      const __returned__ = common_vendor.e({
        a: !isError.value
      }, !isError.value ? {
        b: common_vendor.s(imageStyle.value),
        c: isShow.value ? __props.image : __props.loadingImg,
        d: __props.imgMode,
        e: common_vendor.o(imgLoaded, "59"),
        f: common_vendor.o(loadError, "e0"),
        g: common_vendor.o(clickImg, "5c")
      } : {
        h: common_vendor.s(imageStyle.value),
        i: __props.errorImg,
        j: __props.imgMode,
        k: common_vendor.o(errorImgLoaded, "f5"),
        l: common_vendor.o(clickImg, "a0")
      }, {
        m: common_vendor.n("up-lazy-item-" + common_vendor.unref(elIndex)),
        n: common_vendor.sei(common_vendor.gei(_ctx, ""), "view"),
        o: common_vendor.s(wrapStyle.value),
        p: common_vendor.s({
          "--status-bar-height": `${_ctx.u_s_b_h}px`,
          "--uni-safe-area-inset-bottom": `${_ctx.u_s_a_i_b}px`
        }),
        q: common_vendor.n("up-lazy-item-" + common_vendor.unref(elIndex)),
        r: common_vendor.pvhc(_ctx.$scope.data.virtualHostClass)
      });
      return __returned__;
    };
  }
});
const Component = /* @__PURE__ */ common_vendor._export_sfc(_sfc_main, [["__scopeId", "data-v-d3bc56ce"]]);
wx.createComponent(Component);
//# sourceMappingURL=../../../../../.sourcemap/mp-weixin/uni_modules/uview-ultra/components/up-lazy-load/up-lazy-load.js.map
