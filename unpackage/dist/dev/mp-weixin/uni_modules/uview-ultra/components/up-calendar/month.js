...
const _sfc_main = common_vendor.defineComponent({
  ...
  computed: {
    ...
    daySelectStyle() {
      return (index1, index2, item) => {
        let date = item["dateStr"] != null ? item["dateStr"] : uni_modules_limeDayuts_common_index.dayuts(item["date"]).format("YYYY-MM-DD"), style = new common_vendor.UTSJSONObject(
          {}
    ...
    textStyle() {
      return (item) => {
        const date = item["dateStr"] != null ? item["dateStr"] : uni_modules_limeDayuts_common_index.dayuts(item["date"]).format("YYYY-MM-DD"), style = new common_vendor.UTSJSONObject(
          {}
          // 选中的日期，提示文字设置白色
        );
        const sameDate = (element) => {
