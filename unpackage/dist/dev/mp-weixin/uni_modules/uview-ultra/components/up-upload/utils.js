...
function pickExclude(objOri, keys) {
  ...
  return common_vendor.UTSJSONObject.keys(obj).reduce((prev, key) => {
...
function formatImage(res) {
  return res.tempFiles.map((item) => {
    let tmp = new common_vendor.UTSJSONObject(Object.assign(Object.assign({}, pickExclude(item, ["path"])), { type: "image", url: item.path, thumb: item.path, size: item.size, name: common_vendor.UTS.arrayPop(item.path.split("/")) + ".png" }));
    return common_vendor.UTS.JSON.parseObject(common_vendor.UTS.JSON.stringify(tmp));
  });
}
function formatVideo(res) {
  var _a, _b;
  let tmp = new common_vendor.UTSJSONObject(Object.assign(Object.assign({}, pickExclude(res, ["tempFilePath", "thumbTempFilePath", "errMsg"])), {
    type: "video",
  ...
  return new Promise((resolve, reject) => {
    switch (accept) {
      case "image":
        ...
        break;
      case "media":
        common_vendor.wx$1.chooseMedia(new common_vendor.UTSJSONObject({
          count: multiple ? Math.min(maxCount, 9) : 1,
          sourceType: captureList,
          maxDuration,
          sizeType,
      ...
      case "file":
        ...
        break;
      default:
        common_vendor.wx$1.chooseMessageFile(new common_vendor.UTSJSONObject({
          count: multiple ? maxCount : 1,
          type: "all",
          success: (res = null) => {
            return resolve(formatFile(res));
