...
const _sfc_main = common_vendor.defineComponent({
  ...
  methods: {
    ...
    onChooseTap() {
      this.chooseFile(new common_vendor.UTSJSONObject({}));
    },
    chooseFile(params) {
      ...
      const chooseParams = common_vendor.UTSJSONObject.assign(new common_vendor.UTSJSONObject({
        ...
        camera: this.camera
      }), new common_vendor.UTSJSONObject(Object.assign({ maxCount: maxCount - lists.length }, params)));
    ...
    onBeforeRead(file) {
      ...
      if (useBeforeRead) {
        this.$emit("beforeRead", common_vendor.UTSJSONObject.assign(common_vendor.UTSJSONObject.assign(new common_vendor.UTSJSONObject({
          file: this.convertFileToUTSJSON(file)
        }), this.getDetail(null)), new common_vendor.UTSJSONObject({
          callback: (ok) => {
            if (ok) {
      ...
      return common_vendor.__awaiter(this, void 0, void 0, function* () {
        ...
        if (oversize) {
          common_vendor.index.showToast({
            title: uni_modules_uviewUltra_libs_i18n_index.t("up.upload.sizeExceed", new common_vendor.UTSJSONObject({}))
          });
          this.$emit("oversize", common_vendor.UTSJSONObject.assign(new common_vendor.UTSJSONObject({
            file: this.convertFileToUTSJSON(file)
          }), this.getDetail(null)));
        ...
        if (this.autoUpload) {
          ...
          lists.map((item) => {
            ...
            if (itemJSON != null) {
              fileList.push(common_vendor.UTSJSONObject.assign(itemJSON, new common_vendor.UTSJSONObject({
                status: "uploading",
                message: uni_modules_uviewUltra_libs_i18n_index.t("up.upload.uploading", new common_vendor.UTSJSONObject({})),
                progress: 0
              })));
            }
          });
          ...
          for (let i = 0; i < lists.length; i++) {
            ...
            switch (this.autoUploadDriver) {
              ...
              case "upload_oss":
                console.log();
                let formData = new common_vendor.UTSJSONObject({});
                  ...
                  success: (ret) => {
                    ...
                    if (res0.getString("data.params.host") != null) {
                      ...
                      uploadTask2.onProgressUpdate((res) => {
                        that.updateUpload(len + j, new common_vendor.UTSJSONObject({
          ...
          this.$emit("afterRead", common_vendor.UTSJSONObject.assign(new common_vendor.UTSJSONObject({
            file: this.convertFileToUTSJSON(file)
          }), this.getDetail(null)));
        }
      });
    ...
    emitAfterAutoUpload(res = null, index, defaultUrl = "", defaultThumb = "") {
      const payload = common_vendor.UTSJSONObject.assign(res != null ? res : new common_vendor.UTSJSONObject({}), new common_vendor.UTSJSONObject({
        url: defaultUrl,
        thumb: defaultThumb,
        callback: (r) => {
          var _a, _b;
    ...
    updateUpload(index, param) {
      ...
      fileList.splice(index, 1, new common_vendor.UTSJSONObject(Object.assign(Object.assign({}, item), {
    ...
    succcessUpload(index, url, thumb = "") {
      const fileList = this.fileList;
      let item = fileList[index];
      fileList.splice(index, 1, new common_vendor.UTSJSONObject(Object.assign(Object.assign({}, item), { status: "success", message: "", url, progress: 100, thumb })));
      this.$emit("update:fileList", fileList);
    },
    deleteItem(index) {
      const fileList = this.fileList;
      if (this.autoDelete) {
        ...
        this.$emit("delete", common_vendor.UTSJSONObject.assign(common_vendor.UTSJSONObject.assign(new common_vendor.UTSJSONObject({}), this.getDetail(index)), new common_vendor.UTSJSONObject({
          file: fileList[index]
        })));
      }
    },
    ...
    onPreviewVideo(previewItem, index) {
      ...
      for (var i = 0; i < this.lists.length; i++) {
        ...
        if (item.isVideo || item.type != null && item.type === "video") {
          sources.push(common_vendor.UTSJSONObject.assign(common_vendor.UTSJSONObject.assign(new common_vendor.UTSJSONObject({}), item), new common_vendor.UTSJSONObject({
            type: "video"
          })));
          if (i === index) {
            current = videoIndex;
      ...
      common_vendor.wx$1.previewMedia(new common_vendor.UTSJSONObject({
        sources,
        current,
        fail() {
    ...
    onClickPreview(item, index) {
      ...
      this.$emit(
        "clickPreview",
        // @ts-ignore
        common_vendor.UTSJSONObject.assign(common_vendor.UTSJSONObject.assign(new common_vendor.UTSJSONObject({}), item), this.getDetail(index))
      );
    }
  }
});
