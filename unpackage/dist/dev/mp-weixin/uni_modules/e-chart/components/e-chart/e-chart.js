  ...
  methods: {
    ...
    initMP(option) {
      return common_vendor.__awaiter(this, void 0, void 0, function* () {
        ...
        this.echartObj = uni_modules_eChart_components_eChart_js_echarts_esm_min.xm(canvas, this.theme, new common_vendor.UTSJSONObject({
          devicePixelRatio,
          width,
          height
        }));
        ...
        return new common_vendor.UTSJSONObject({ echartCanvas: node, echartObj: this.echartObj, canvasId: this.canvasId, width, height });
      });
    },
    // 获取echart实例
    getEchartObj() {
    ...
    resize() {
      ...
      this._resizeTimer = setTimeout(() => {
        this.getCanvasQuery().boundingClientRect().exec((res) => {
          ...
          this.echartObj.resize(new common_vendor.UTSJSONObject({ width, height }));
        });
      }, 30);
    },
    // 获取canvas查询对象
    ...
    getMpCanvas() {
      return new Promise((resolve) => {
        this.getCanvasQuery().fields(new common_vendor.UTSJSONObject({ node: true, size: true })).exec((res) => {
    ...
    canvasToTempFilePath() {
      return new Promise((resolve, reject) => {
        ...
        const option = this.echartCanvas ? new common_vendor.UTSJSONObject({ canvas: this.echartCanvas }) : new common_vendor.UTSJSONObject({ canvasId: this.canvasId });
