...
class RsaPublicKey extends common_vendor.UTS.UTSType {
...
class RsaPrivateKey extends common_vendor.UTS.UTSType {
  static get$UTSMetadata$() {
    return {
      kind: 2,
      get fields() {
...
class DerElement extends common_vendor.UTS.UTSType {
  static get$UTSMetadata$() {
    return {
      kind: 2,
      get fields() {
...
function base64DecodeToBytes(b64) {
  ...
  return new Uint8Array(common_vendor.index.base64ToArrayBuffer(clean));
}
...
function isProbablePrime(n = null, k = 5) {
  ...
  for (let i = 0; i < smallPrimes.length; i++) {
    const p = bigIntFunc(smallPrimes[i]);
    if (n % p == zero) {
      return n == p;
    }
  }
  for (let i = 0; i < k; i++) {
    const a = two + bigIntFunc(Math.floor(Math.random() * 1e4)) % (n - four);
...
function rsaGenerateKeyPairJs(bits) {
  ...
  const pubB64 = common_vendor.index.arrayBufferToBase64(spkiDer.buffer);
  ...
  const priB64 = common_vendor.index.arrayBufferToBase64(pkcs8Der.buffer);
  return '{"publicKey":"' + pubB64 + '","privateKey":"' + priB64 + '"}';
}
function rsaGenerateKeyPairUts(bits) {
  return rsaGenerateKeyPairJs(bits);
}
exports.rsaDecryptUts = rsaDecryptUts;
exports.rsaEncryptUts = rsaEncryptUts;
exports.rsaGenerateKeyPairUts = rsaGenerateKeyPairUts;
exports.rsaSignUts = rsaSignUts;
exports.rsaVerifyUts = rsaVerifyUts;
//# sourceMappingURL=../../../.sourcemap/mp-weixin/uni_modules/unix-crypto/rsa.js.map
