...
if (!Math) {
  (_easycom_NavBar + _easycom_up_upload + _easycom_up_button + common_vendor.unref(AppKu))();
}
  ...
  setup(__props) {
    ...
    function afterRead(event) {
      return common_vendor.__awaiter(this, void 0, void 0, function* () {
        ...
        lists.forEach((item) => {
          fileList1.value.push(new common_vendor.UTSJSONObject(Object.assign(Object.assign({}, item), { status: "uploading", message: "上传中" })));
        });
        for (let i = 0; i < lists.length; i++) {
          const url = lists[i].getString("url");
          const result = yield uploadFilePromise(url);
          const item = fileList1.value[fileListLen];
          fileList1.value.splice(fileListLen, 1, Object.assign(item, new common_vendor.UTSJSONObject({
            status: "success",
            message: "",
            url: result
          })));
    ...
    return (_ctx, _cache) => {
      ...
      const __returned__ = {
        ...
        b: common_vendor.o(afterRead, "71"),
        c: common_vendor.o(deletePic, "31"),
        d: common_vendor.p({
