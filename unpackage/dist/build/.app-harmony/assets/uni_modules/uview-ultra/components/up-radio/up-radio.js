...
import { a as addUnit, q as os, d as deepMerge, b as addStyle } from "../../libs/function/index.js";
...
const _sfc_main = /* @__PURE__ */ _defineVaporSharedDataComponent({
  ...
  setup(__props, _a) {
    ...
    const elLabelLineHeight = computed(() => {
      const size = elLabelSize.value;
      const num = parseFloat(size);
      if (isNaN(num)) {
        return size;
      }
      const unit = size.indexOf("rpx") != -1 ? "rpx" : "px";
      return Math.round(num * 1.35).toString() + unit;
    });
    ...
    return () => {
      ...
      _renderSharedDataEffect(() => {
        _setSharedDataClass(__sharedData, 8, ["weapp-tw-border up-radio wtu-dmb0mx-0", [iconPlacementClass.value, borderBottomClass.value]]);
        _setSharedDataStyle(__sharedData, 9, [radioStyle.value]);
        _setSharedDataClass(__sharedData, 10, ["weapp-tw-border up-radio__icon-wrap wtu-dmb0mx-0", iconClasses.value]);
        _setSharedDataStyle(__sharedData, 11, [iconWrapStyle.value]);
      });
      _createSharedDataSlot("icon", null, null, () => {
        const n2 = _createSharedDataComponentWithFallback(_component_up_icon, "90b95ea8", {
      ...
      _createSharedDataSlot("label", {
        ...
        _renderSharedDataEffect(() => {
          ...
          _setSharedDataClass(__sharedData, 2, ["weapp-tw-border up-radio__label", [_elDisabled ? "up-radio__label--disabled" : ""]]);
          _setSharedDataStyle(__sharedData, 3, {
            color: _elDisabled ? elInactiveColor.value : elLabelColor.value,
            fontSize: elLabelSize.value,
            lineHeight: elLabelLineHeight.value
          });
