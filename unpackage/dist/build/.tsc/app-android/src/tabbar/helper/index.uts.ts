...
import { isCompilerVersionGte, safeAreaInsets, TABBAR_BASE_HEIGHT } from '@/src/utils/systemInfo';
...
/** 是否启用 tabbar 缓存（原生或自定义模式下为 true，无 tabbar 时为 false） */
export const tabbarCacheEnable: boolean = selectedTabbarStrategy != TABBAR_STRATEGY_MAP.NO_TABBAR;

/** 是否启用自定义 tabbar（模式 2 或 模式 3 为 true） */
...
/** 是否为无全局 TabBar 模式（模式 0 为 true） */
export const isNoTabbar: boolean = selectedTabbarStrategy == TABBAR_STRATEGY_MAP.NO_TABBAR;

/** 是否为悬浮胶囊风格 TabBar（根据 config.uts 中的 type 判定） */
export const isCapsuleTabbar: boolean = customTabbarConfig.type == 'capsule';
...
/** 是否具有 pages.json 原生配置（模式 1 和 模式 2 为 true） */
export const hasNativeTabbarConfig: boolean = selectedTabbarStrategy == TABBAR_STRATEGY_MAP.NATIVE_TABBAR || selectedTabbarStrategy == TABBAR_STRATEGY_MAP.CUSTOM_TABBAR_WITH_NATIVE;
