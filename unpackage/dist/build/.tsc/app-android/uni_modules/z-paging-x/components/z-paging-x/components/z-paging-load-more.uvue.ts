...
return (): any | null => {
  ...
  return _cE("view", _uM({
    class: "weapp-tw-border zpx-l-container",
    onClick: onClickMore
  }), [
    isTrue(isNoMore.value)
      ? _cE("view", _uM({
          key: 0,
          class: "weapp-tw-border zpx-l-line"
        }))
      : _cC("v-if", true),
    isTrue(isLoading.value)
      ? _cV(unref(zPagingLoading), _uM({ key: 1 }))
      : _cC("v-if", true),
    _cE("text", _uM({ class: "weapp-tw-border zpx-l-loading-text" }), _tD(statusText.value), 1 /* TEXT */),
    isTrue(isNoMore.value)
      ? _cE("view", _uM({
          key: 2,
          class: "weapp-tw-border zpx-l-line"
