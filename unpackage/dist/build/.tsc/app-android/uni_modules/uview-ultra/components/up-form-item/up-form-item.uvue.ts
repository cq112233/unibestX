...
import type { PropType } from 'vue'
	...
	import { addStyle, addUnit, getProperty, setProperty, error, deepMerge } from '../../libs/function/index.uts'
...
__ins.emit(event, ...do_not_transform_spread)
	...
	const bodyStyle = computed((): UTSJSONObject => {
		const custom = addStyle(props.customStyle) as UTSJSONObject
		const parentPos = parentData.value != null ? (parentData.value['labelPosition'] as string | null) : null
		const pos = props.labelPosition != '' ? props.labelPosition : (parentPos ?? 'left')
		const base = {
	...
	const labelWrapperStyle = computed((): UTSJSONObject => {
		const parentPos = parentData.value != null ? (parentData.value['labelPosition'] as string | null) : null
		const pos = props.labelPosition != '' ? props.labelPosition : (parentPos ?? 'left')
		const parentWidth = parentData.value != null ? (parentData.value['labelWidth'] ?? 45) : 45
	...
	const labelAlignStyle = computed((): UTSJSONObject => {
		const align = parentData.value != null ? (parentData.value['labelAlign'] as string | null) : null
		return {
			justifyContent: align == 'left' ? 'flex-start' : align == 'center' ? 'center' : 'flex-end'
		} as UTSJSONObject
	})

	const labelCustomStyle = computed((): UTSJSONObject => {
	...
	const isErrorMessage = computed((): boolean => {
		const errorType = parentData.value != null ? (parentData.value['errorType'] as string | null) : null
		return message.value != '' && errorType == 'message'
	})

	const lineColor = computed((): string => {
		const errorType = parentData.value != null ? (parentData.value['errorType'] as string | null) : null
		return message.value != '' && errorType == 'border-bottom' ? (color['error'] as string) : ((propsLine['color'] ?? '#d6d7d9') as string)
	})
...
const _component_up_line = resolveEasyComponent("up-line",_easycom_up_line)
  ...
  return _cE("view", _uM({
    class: _nC(["weapp-tw-border up-form-item", _uM({'up-form-item--error': isErrorMessage.value})])
  }), [
    _cE("view", _uM({
      class: "weapp-tw-border up-form-item__body",
      ...
      renderSlot(_ctx.$slots, "label", {}, (): any[] => [
        isTrue(_ctx.required || _ctx.leftIcon != '' || _ctx.label != '')
          ? _cE("view", _uM({
              key: 0,
              class: "weapp-tw-border up-form-item__body__left",
              style: _nS(labelWrapperStyle.value)
            }), [
              _cE("view", _uM({ class: "weapp-tw-border up-form-item__body__left__content" }), [
                isTrue(_ctx.required)
                  ? _cE("text", _uM({
                      key: 0,
                      class: "weapp-tw-border up-form-item__body__left__content__required"
                    }), "*")
                  : _cC("v-if", true),
                _ctx.leftIcon != ''
                  ? _cE("view", _uM({
                      key: 1,
                      class: "weapp-tw-border up-form-item__body__left__content__icon"
                    }), [
                  ...
                  : _cC("v-if", true),
                _cE("view", _uM({
                  class: "weapp-tw-border up-form-item__body__left__content__label",
                  style: _nS(labelAlignStyle.value)
                }), [
                  _cE("text", _uM({
                    class: "weapp-tw-border up-form-item__body__left__content__label__text",
                    style: _nS(labelCustomStyle.value)
      ...
      _cE("view", _uM({ class: "weapp-tw-border up-form-item__body__right" }), [
        _cE("view", _uM({ class: "weapp-tw-border up-form-item__body__right__content" }), [
          _cE("view", _uM({ class: "weapp-tw-border up-form-item__body__right__content__slot" }), [
            renderSlot(_ctx.$slots, "default")
          ]),
          _ctx.$slots['right'] != null
            ? _cE("view", _uM({
                key: 0,
                class: "weapp-tw-border item__body__right__content__icon"
              }), [
    ...
    renderSlot(_ctx.$slots, "error", {}, (): any[] => [
      isTrue(isErrorMessage.value)
        ? _cE("text", _uM({
            key: 0,
            class: "weapp-tw-border up-form-item__body__right__message",
