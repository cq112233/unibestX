			...
			async onAfterRead(file: Array<UPUploadFileChoosed|null>) {
				...
				if (this.autoUpload) {
					...
					lists.map((item: UPUploadFileChoosed) => {
						// @ts-ignore
						const itemJSON = JSON.parseObject(JSON.stringify(item))
						if (itemJSON != null) {
