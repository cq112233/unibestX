...
const _component_up_badge = resolveEasyComponent("up-badge",_easycom_up_badge)
  ...
  return _cE("view", _uM({
    class: _nC(["weapp-tw-border up-tabbar-item", [isMidButton.value ? 'up-tabbar-item--mid-button' : '']]),
    style: _nS(customTabbarItemStyle.value),
    onClick: clickHandler
  }), [
    _cE("view", _uM({
      class: _nC(["weapp-tw-border up-tabbar-item__icon", [isMidButton.value ? 'up-tabbar-item__icon--mid-button' : '']])
    }), [
      isTrue(isMidButton.value)
        ? _cE("view", _uM({
            key: 0,
            class: "weapp-tw-border up-tabbar-item--mid-button-cover"
          }))
        : _cC("v-if", true),
      _ctx.icon != ''
    ...
    renderSlot(_ctx.$slots, "text", {}, (): any[] => [
      _cE("text", _uM({
        class: "weapp-tw-border up-tabbar-item__text",
        style: _nS(_uM({
					color: isActive.value ? activeColor.value : inactiveColor.value
				}))
