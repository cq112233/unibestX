...
const _component_up_loading_icon = resolveEasyComponent("up-loading-icon",_easycom_up_loading_icon)
  ...
  return _cE("view", _uM({
    class: "weapp-tw-border up-loadmore",
    style: _nS(loadmoreStyle.value)
  }), [
    isTrue(_ctx.line)
      ...
      : _cC("v-if", true),
    _cE("view", _uM({
      class: _nC([_ctx.status == 'loadmore' || _ctx.status == 'nomore' ? 'up-more' : '', "weapp-tw-border up-loadmore__content"])
    }), [
      isTrue(_ctx.status == 'loading' && _ctx.icon)
        ? _cE("view", _uM({
            key: 0,
            class: "weapp-tw-border up-loadmore__content__icon-wrap"
          }), [
        ...
        : _cC("v-if", true),
      _cE("text", _uM({
        class: _nC(["weapp-tw-border up-line-11", [(_ctx.status == 'nomore' && _ctx.isDot == true) ? 'up-loadmore__content__dot-text' : 'up-loadmore__content__text']]),
        style: _nS(loadTextStyle.value),
        onClick: loadMore
      }), _tD(showText.value), 7 /* TEXT, CLASS, STYLE */)
    ], 2 /* CLASS */),
    isTrue(_ctx.line)
      ? _cV(_component_up_line, _uM({
          key: 1,
