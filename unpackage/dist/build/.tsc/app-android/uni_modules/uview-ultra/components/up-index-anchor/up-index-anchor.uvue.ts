...
__ins.emit(event, ...do_not_transform_spread)
	...
	const parentSticky = computed((): boolean => {
		...
		if (parentData.value['wtu-ok2e2g-0'] != null) {
			return indexList != null ? parentData.value['wtu-ok2e2g-0'] as boolean : true
		} else {
			return false
		}
	})

	const anchorStyle = computed((): UTSJSONObject => {
		return {
...
return (): any | null => {
  ...
  return _cE("view", _uM({
    class: _nC(["weapp-tw-border up-index-anchor up-border-bottom", _uM({ 'up-index-anchor--sticky': parentSticky.value })]),
    ref: `up-index-anchor-${textName.value}`,
    style: _nS(anchorStyle.value)
  }), [
    _cE("text", _uM({
      class: "weapp-tw-border up-index-anchor__text",
      style: _nS(textStyle.value)
    }), _tD(textName.value), 5 /* TEXT, STYLE */)
  ], 6 /* CLASS, STYLE */)
}
}
