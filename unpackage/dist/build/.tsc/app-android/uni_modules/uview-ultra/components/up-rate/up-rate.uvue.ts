...
name: 'up-rate',
  props: {
    modelValue: { type: [String, Number], required: false, default: 1 },
    count: { type: [String, Number], required: false, default: 5 },
    disabled: { type: Boolean, required: false, default: false },
    readonly: { type: Boolean, required: false, default: false },
    size: { type: [String, Number], required: false, default: 18 },
    inactiveColor: { type: String, required: false, default: '#b2b2b2' },
    activeColor: { type: String, required: false, default: '#FA3534' },
    inactiveIcon: { type: String, required: false, default: 'star-fill' },
    activeIcon: { type: String, required: false, default: 'star-fill' },
    gutter: { type: [String, Number], required: false, default: 4 },
    minCount: { type: [String, Number], required: false, default: 0 },
    allowHalf: { type: Boolean, required: false, default: false },
    touchable: { type: Boolean, required: false, default: true },
...
const _component_up_icon = resolveEasyComponent("up-icon",_easycom_up_icon)
  ...
  return _cE("view", _uM({
    class: "weapp-tw-border up-rate",
    id: elId.value,
    style: _nS(customRateStyle.value)
  }), [
    _cE("view", _uM({
      class: "weapp-tw-border up-rate__content",
      onTouchmove: withModifiers(touchMove, ["stop"]),
      onTouchend: withModifiers(touchEnd, ["stop"])
    }), [
      _cE(Fragment, null, RenderHelpers.renderList(parseInt(_ctx.count.toString()), (_, index, __index, _cached): any => {
        return _cE("view", _uM({
          class: _nC(["weapp-tw-border up-rate__content__item wtu-1h9eaf1-0", [elClass.value]]),
          key: index
        }), [
          _cE("view", _uM({
            class: "weapp-tw-border up-rate__content__item__icon-wrap",
            onClick: withModifiers(($event: any) => {clickHandler($event, index + 1)}, ["stop"])
          }), [
            _cV(_component_up_icon, _uM({
              name: 
                            Math.floor(parseFloat(activeIndex.value.toString())) > index
                                ? _ctx.activeIcon
                                : _ctx.inactiveIcon
                        ,
              color: 
                            _ctx.disabled
                                ? '#c8c9cc'
                                : Math.floor(parseFloat(activeIndex.value.toString())) > index
                                ? (_ctx.activeColor == '#FA3534' ? 'primary' : _ctx.activeColor)
                                : _ctx.inactiveColor
                        ,
              "custom-style": iconPaddingStyle.value,
              size: _ctx.size
            }), null, 8 /* PROPS */, ["name", "color", "custom-style", "size"])
          ], 8 /* PROPS */, ["onClick"]),
          isTrue(_ctx.allowHalf)
            ? _cE("view", _uM({
                key: 0,
                onClick: withModifiers(($event: any) => {clickHandler($event, index + 1)}, ["stop"]),
                class: "weapp-tw-border up-rate__content__item__icon-wrap up-rate__content__item__icon-wrap--half",
                style: _nS(halfWidthStyle.value)
              }), [
                _cV(_component_up_icon, _uM({
                  name: 
							Math.floor(parseFloat(activeIndex.value.toString())) > index
                  ...
                  color: 
                            _ctx.disabled
                                ? '#c8c9cc'
                                : Math.ceil(parseFloat(activeIndex.value.toString())) > index
                                ? (_ctx.activeColor == '#FA3534' ? 'primary' : _ctx.activeColor)
                                : _ctx.inactiveColor
                        ,
                  "custom-style": iconPaddingStyle.value,
...
export type UpRateComponentPublicInstance = InstanceType<typeof __sfc__>;
const GenUniModulesUviewUltraComponentsUpRateUpRateStyles = [_uM([["weapp-tw-border", _pS(_uM([["borderTopWidth", 0], ["borderRightWidth", 0], ["borderBottomWidth", 0], ["borderLeftWidth", 0]]))], ["up-rate", _pS(_uM([["display", "flex"], ["flexDirection", "row"], ["alignItems", "center"], ["marginTop", 0], ["marginRight", 0], ["marginBottom", 0], ["marginLeft", 0], ["paddingTop", 0], ["paddingRight", 0], ["paddingBottom", 0], ["paddingLeft", 0]]))], ["up-rate__content", _pS(_uM([["display", "flex"], ["flexDirection", "row"]]))], ["up-rate__content__item", _pS(_uM([["position", "relative"]]))], ["up-rate__content__item__icon-wrap--half", _pS(_uM([["position", "absolute"], ["overflow", "hidden"], ["top", 0], ["left", 0]]))], ["up-icon", _pS(_uM([["boxSizing", "border-box"]]))], ["wtu-1h9eaf1-0", _pS(_uM([["cursor", "pointer"]]))]])]
