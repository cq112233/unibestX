...
const _cache = __ins.renderCache;
	...
	const customCellGroupStyle = computed((): UTSJSONObject => {
...
const _component_up_line = resolveEasyComponent("up-line",_easycom_up_line)
  ...
  return _cE("view", _uM({
    style: _nS(customCellGroupStyle.value),
    class: _nC([[_ctx.customClass], "weapp-tw-border up-cell-group"])
  }), [
    isTrue(_ctx.title)
      ? _cE("view", _uM({
          key: 0,
          class: "weapp-tw-border up-cell-group__title"
        }), [
          renderSlot(_ctx.$slots, "title", {}, (): any[] => [
            _cE("text", _uM({ class: "weapp-tw-border up-cell-group__title__text" }), _tD(_ctx.title), 1 /* TEXT */)
          ])
        ])
      : _cC("v-if", true),
    _cE("view", _uM({ class: "weapp-tw-border up-cell-group__wrapper" }), [
      isTrue(_ctx.border)
