...
// 计算属性
const alertStyle = computed((): UTSJSONObject => {
...
const _component_up_transition = resolveEasyComponent("up-transition",_easycom_up_transition)
  ...
  return _cV(_component_up_transition, _uM({
    ...
    default: withSlotCtx((): any[] => [
      _cE("view", _uM({
        class: _nC(["weapp-tw-border up-alert", [`up-alert--${_ctx.type}--${_ctx.effect}`]]),
        onClick: withModifiers(clickHandler, ["stop"]),
        style: _nS(alertStyle.value)
      }), [
        isTrue(_ctx.showIcon)
          ? _cE("view", _uM({
              key: 0,
              class: "weapp-tw-border up-alert__icon"
            }), [
          ...
          : _cC("v-if", true),
        _cE("view", _uM({
          class: "weapp-tw-border up-alert__content",
          style: _nS(_uM({
					paddingRight: _ctx.closable ? '20px' : '0px'
				}))
        }), [
          isTrue(_ctx.title)
            ? _cE("text", _uM({
                key: 0,
                class: _nC(["weapp-tw-border up-alert__content__title", [_ctx.effect === 'dark' ? 'up-alert__text--dark' : `up-alert__text--${_ctx.type}--light`]]),
          ...
          isTrue(_ctx.description)
            ? _cE("text", _uM({
                key: 1,
                class: _nC(["weapp-tw-border up-alert__content__desc", [_ctx.effect === 'dark' ? 'up-alert__text--dark' : `up-alert__text--${_ctx.type}--light`]]),
                style: _nS(style1.value)
        ...
        isTrue(_ctx.closable)
          ? _cE("view", _uM({
              key: 1,
              class: "weapp-tw-border up-alert__close",
