...
// 方法 - 图片样式
function getImageStyle(index1: number, index2: number): UTSJSONObject {
...
// 计算属性 - 图片圆角
const imageBorderRadius = computed((): string => {
    return props.shape == 'circle' ? '10000px' : addUnit(props.radius)
})

// 计算属性 - 查看更多圆角
const moreBorderRadius = computed((): string => {
    return props.shape == 'circle' ? '50%' : addUnit(props.radius)
})
...
const rowWrapStyle = computed((): UTSJSONObject => {
    return { flexWrap: props.autoWrap ? 'wrap' : 'nowrap' } as UTSJSONObject
})

const imageStyleComputed = computed((): UTSJSONObject => {
    return {
...
const imageModeComputed = computed((): string => {
    if (props.urls.length == 1) {
        return (imageHeight.value != '0px' && imageHeight.value != '0rpx')
            ? props.singleMode
            : 'widthFix'
    }
...
// 预览图片
const onPreviewTap = (e: any, url: string) => {
...
const _component_up_text = resolveEasyComponent("up-text",_easycom_up_text)
  ...
  return _cE("view", _uM({ class: "weapp-tw-border up-album" }), [
    _cE(Fragment, null, RenderHelpers.renderList(showUrls.value, (arr, index, __index, _cached): any => {
      return _cE("view", _uM({
        class: "weapp-tw-border up-album__row",
        ref_for: true,
        ref: "up-album__row",
        key: index,
        style: _nS(rowWrapStyle.value)
      }), [
        _cE(Fragment, null, RenderHelpers.renderList(arr, (item, index1, __index, _cached): any => {
          return _cE("view", _uM({
            class: "weapp-tw-border up-album__row__wrapper",
            key: index1,
            style: _nS(getImageStyle(index + 1, index1 + 1)),
            onClick: ($event: any) => {onPreviewTap($event, getSrc(item))}
          }), [
            _cE("image", _uM({
              src: getSrc(item),
              mode: imageModeComputed.value,
              style: _nS(imageStyleComputed.value)
            }), null, 12 /* STYLE, PROPS */, ["src", "mode"]),
            isTrue(
                        _ctx.showMore &&
                        _ctx.urls.length > parseInt(_ctx.rowCount.toString()) * showUrls.value.length &&
                        index === showUrls.value.length - 1 &&
                        index1 === showUrls.value[showUrls.value.length - 1].length - 1
                    )
              ? _cE("view", _uM({
                  key: 0,
                  class: "weapp-tw-border up-album__row__wrapper__text",
                  style: _nS(_uM({
					    borderRadius: moreBorderRadius.value,
				    }))
                }), [
                  _cV(_component_up_text, _uM({
                    text: `+${_ctx.urls.length - parseInt(_ctx.maxCount.toString())}`,
                    color: "#fff",
                    size: parseInt(_ctx.multipleSize.toString()) * 0.3,
                    align: "center",
                    customStyle: "justify-content: center"
                  }), null, 8 /* PROPS */, ["text", "size"])
                ], 4 /* STYLE */)
              : _cC("v-if", true)
