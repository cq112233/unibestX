    ...
    companion object {
        ...
        var setup: (__props: GenUniModulesUviewUltraComponentsUpIconUpIcon) -> Any? = fun(__props): Any? {
            ...
            val uClasses = computed(fun(): String {
                ...
                var types: UTSArray<String>? = default__11.getArray("type") as UTSArray<String>
            ...
            val mergedImgStyle = computed(fun(): UTSJSONObject {
                val custom = addStyle(props.customStyle) as UTSJSONObject
                val img = imgStyle.value as UTSJSONObject
                return deepMerge(img, custom) as UTSJSONObject
            }
            )
            val mergedIconStyle = computed(fun(): UTSJSONObject {
                val custom = addStyle(props.customStyle) as UTSJSONObject
                val iconS = iconStyle.value as UTSJSONObject
                return deepMerge(iconS, custom) as UTSJSONObject
            }
            )
            val labelStyleComputed = computed(fun(): UTSJSONObject {
                return _uO("color" to props.labelColor, "fontSize" to addUnit(props.labelSize), "marginLeft" to if (props.labelPos == "right") {
                    addUnit(props.space)
                } else {
                    "0px"
                }
                , "marginTop" to if (props.labelPos == "bottom") {
                    addUnit(props.space)
                } else {
                    "0px"
                }
                , "marginRight" to if (props.labelPos == "left") {
                    addUnit(props.space)
                } else {
                    "0px"
                }
                , "marginBottom" to if (props.labelPos == "top") {
                    addUnit(props.space)
                } else {
                    "0px"
                }
                ...
                return _cE("view", _uM("class" to _nC(_uA(
                    ...
                    if (isTrue(isImg.value)) {
                        _cE("image", _uM("key" to 0, "class" to "up-icon__img", "src" to _ctx.name, "mode" to if (_ctx.imgMode != "") {
                            ...
                            "aspectFit"
                        }, "style" to _nS(mergedImgStyle.value)), null, 12, _uA(
                            "src",
                            "mode"
                        ))
                    } else {
                        _cE("text", _uM("key" to 1, "class" to _nC(_uA(
                            "weapp-tw-border up-icon__icon",
                            uClasses.value
                        )), "style" to _nS(mergedIconStyle.value), "hover-class" to _ctx.hoverClass), _tD(icon.value), 15, _uA(
                            "hover-class"
                        ))
                    }
                    ,
                    if (_ctx.label !== "") {
                        _cE("text", _uM("key" to 2, "class" to "weapp-tw-border up-icon__label", "style" to _nS(labelStyleComputed.value)), _tD(_ctx.label), 5)
                    } else {
                        _cC("v-if", true)
                    }
                ), 2)
        ...
        ), "customClass" to _uM("type" to "String", "default" to ""), "name" to _uM("type" to "String", "default" to fun(): String? {
