...
open class GenUniModulesUviewUltraComponentsUpImageUpImage : VueComponent {
    ...
    companion object {
        ...
        var setup: (__props: GenUniModulesUviewUltraComponentsUpImageUpImage, __setupCtx: SetupContext) -> Any? = fun(__props, __setupCtx): Any? {
            ...
            val realSrc = computed(fun(): String {
                if (props.src == "") {
                    return ""
                }
                return props.src
            ...
            val mergedCustomStyle = computed(fun(): UTSJSONObject {
                return addStyle(props.customStyle) as UTSJSONObject
            }
            )
            val imageStyle = computed(fun(): UTSJSONObject {
                return _uO("borderRadius" to if (props.shape == "circle") {
                    "10000px"
                } else {
                    addUnit(props.radius)
                }
                , "width" to addUnit(props.width), "height" to addUnit(props.height))
            }
            )
            val loadingStyle = computed(fun(): UTSJSONObject {
                return _uO("borderRadius" to if (props.shape == "circle") {
                    "50%"
                } else {
                    addUnit(props.radius)
                }
                , "backgroundColor" to props.bgColor, "width" to addUnit(props.width), "height" to addUnit(props.height))
            }
            )
            val errorStyle = computed(fun(): UTSJSONObject {
                return _uO("borderRadius" to if (props.shape == "circle") {
                    "50%"
                } else {
                    addUnit(props.radius)
            ...
            val initLoading = fun(src: String){
                if (src == "") {
                    isError.value = true
                    loading.value = false
                } else {
            ...
            return fun(): Any? {
                ...
                return _cE("view", _uM("class" to "weapp-tw-border up-image", "onClick" to onClick, "style" to _nS(_uA(
                    wrapStyle.value,
                    backgroundStyle.value,
                    mergedCustomStyle.value
                ))), _uA(
                    _cE("image", _uM("src" to realSrc.value, "mode" to _ctx.mode, "onError" to onErrorHandler, "onLoad" to onLoadHandler, "lazy-load" to _ctx.lazyLoad, "class" to "up-image__image", "style" to _nS(imageStyle.value)), null, 44, _uA(
                        "src",
                        "mode",
                        "lazy-load"
                    )),
                    if (isTrue(if (isTruthy(_ctx.showLoading)) {
                        ...
                        _cE("view", _uM("key" to 0, "class" to "weapp-tw-border up-image__loading", "style" to _nS(loadingStyle.value)), _uA(
                    ...
                    if (isTrue((if (isTruthy(_ctx.showError)) {
                        ...
                        _cE("view", _uM("key" to 1, "class" to "weapp-tw-border up-image__error", "style" to _nS(errorStyle.value)), _uA(
        ...
        var props = _nP(_uM("customClass" to _uM("type" to "String", "default" to ""), "url" to _uM("type" to "String", "default" to ""), "linkType" to _uM("type" to "String", "default" to "navigateTo"), "src" to _uM("type" to "String", "default" to fun(): String? {
            return default__25.getString("image.src")
        }
        ), "mode" to _uM("type" to "String", "default" to fun(): String? {
            return default__25.getString("image.mode")
        }
        ), "width" to _uM("type" to "String", "default" to fun(): String? {
            return default__25.getString("image.width")
        }
        ), "height" to _uM("type" to "String", "default" to fun(): String? {
            return default__25.getString("image.height")
        }
        ), "shape" to _uM("type" to "String", "default" to fun(): String? {
            return default__25.getString("image.shape")
        }
        ), "radius" to _uM("type" to "String", "default" to fun(): String? {
            return default__25.getString("image.radius")
        }
        ), "lazyLoad" to _uM("type" to "Boolean", "default" to fun(): Boolean? {
            return default__25.getBoolean("image.lazyLoad")
        }
        ), "showMenuByLongpress" to _uM("type" to "Boolean", "default" to fun(): Boolean? {
            return default__25.getBoolean("image.showMenuByLongpress")
        }
        ), "loadingIcon" to _uM("type" to "String", "default" to fun(): String? {
            return default__25.getString("image.loadingIcon")
        }
        ), "errorIcon" to _uM("type" to "String", "default" to fun(): String? {
            return default__25.getString("image.errorIcon")
        }
        ), "showLoading" to _uM("type" to "Boolean", "default" to fun(): Boolean? {
            return default__25.getBoolean("image.showLoading")
        }
        ), "showError" to _uM("type" to "Boolean", "default" to fun(): Boolean? {
            return default__25.getBoolean("image.showError")
        }
        ), "fade" to _uM("type" to "Boolean", "default" to fun(): Boolean? {
            return default__25.getBoolean("image.fade")
        }
        ), "webp" to _uM("type" to "Boolean", "default" to fun(): Boolean? {
            return default__25.getBoolean("image.webp")
        }
        ), "duration" to _uM("type" to "Number", "default" to fun(): Number? {
            return default__25.getNumber("image.duration")
        }
        ), "bgColor" to _uM("type" to "String", "default" to fun(): String? {
            return default__25.getString("image.bgColor")
        }
        ), "customStyle" to _uM("type" to _uA(
            "Object",
            "String"
