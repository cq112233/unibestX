...
open class GenUniModulesUviewUltraComponentsUpCollapseUpCollapse : VueComponent {
    ...
    open var onChange: (target: ComponentPublicInstance) -> Unit
        get() {
            return unref(this.`$exposed`["onChange"]) as (target: ComponentPublicInstance) -> Unit
        }
        set(value) {
            setRefValue(this.`$exposed`, "onChange", value)
        }
    ...
    companion object {
        ...
        var setup: (__props: GenUniModulesUviewUltraComponentsUpCollapseUpCollapse, __setupCtx: SetupContext) -> Any? = fun(__props, __setupCtx): Any? {
            val __expose = __setupCtx.expose
            val __ins = getCurrentInstance()!!
            val _ctx = __ins.proxy as GenUniModulesUviewUltraComponentsUpCollapseUpCollapse
            ...
            val parentDataSelf = computed(fun(): UTSJSONObject {
                return _uO("accordion" to props.accordion, "border" to props.border, "value" to props.value)
            }
            )
            val init = fun() {
            ...
            val onChange = fun(target: ComponentPublicInstance) {
                val changeArr = _uA<UTSJSONObject>()
                children.value?.map(fun(child: ComponentPublicInstance, index: Number){
                    var childState = child.`$callMethod`("getInternalState") as UTSJSONObject
                    if (props.accordion) {
                        if (child !== target) {
                    ...
                    changeArr.push(_uO("name" to if (childState["name"] != null) {
                        childState["name"].toString()
                    } else {
                        index.toString(10)
                    }
                    , "status" to if (childState["expanded"] as Boolean) {
                        "open"
                    } else {
                        "close"
                    }
                ...
                var trgetState = target.`$callMethod`("getInternalState") as UTSJSONObject
                val expand = trgetState["expanded"] as Boolean
                emit(if (expand) {
                    "open"
                } else {
                    "close"
                }
                , if (trgetState["name"] != null) {
                    trgetState["name"]
                } else {
                    ""
                }
            ...
            __expose(_uM("onChange" to onChange, "getChildren" to getChildren, "addChild" to addChild, "getProps" to getProps, "getRefs" to getRefs))
            return fun(): Any? {
                val _component_up_line = resolveEasyComponent("up-line", GenUniModulesUviewUltraComponentsUpLineUpLineClass)
                return _cE("view", _uM("class" to "weapp-tw-border up-collapse"), _uA(
        ...
        var emits: Map<String, Any?> = _uM("change" to null, "open" to null, "close" to null)
        var props = _nP(_uM("customStyle" to _uM("type" to _uA(
            ...
            return default__31.getString("collapse.value")
        }
        ), "accordion" to _uM("type" to "Boolean", "default" to default__31.getBoolean("collapse.accordion")), "border" to _uM("type" to "Boolean", "default" to default__31.getBoolean("collapse.border"))))
