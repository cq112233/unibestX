...
open class GenUniModulesUviewUltraComponentsUpPaginationUpPagination : VueComponent {
    ...
    companion object {
        ...
        var setup: (__props: GenUniModulesUviewUltraComponentsUpPaginationUpPagination) -> Any? = fun(__props): Any? {
            val __ins = getCurrentInstance()!!
            val _ctx = __ins.proxy as GenUniModulesUviewUltraComponentsUpPaginationUpPagination
            val _cache = __ins.renderCache
            val props = __props
            fun emit(event: String, vararg do_not_transform_spread: Any?) {
                __ins.emit(event, *do_not_transform_spread)
            }
            val totalPages = computed<Number>(fun(): Number {
                val t = parseFloat(props.total.toString(10))
                val p = parseFloat(props.pageSize.toString(10))
            ...
            val shouldShow = computed<Boolean>(fun(): Boolean {
            ...
            val showPager = computed<Boolean>(fun(): Boolean {
            ...
            val showTotal = computed<Boolean>(fun(): Boolean {
                if (props.layout == null) {
                    return false
                }
            ...
            val currentPageStr = computed<String>(fun(): String {
                return props.currentPage.toString(10)
            }
            )
            val displayedPages = computed<UTSArray<String>>(fun(): UTSArray<String> {
                val total = totalPages.value
                val current = props.currentPage
            ...
            fun gen_emitChange_fn(page: Number) {
            ...
            fun gen_goTo_fn(page: String) {
                if (page == "..." || page == props.currentPage.toString(10)) {
                    return
                }
            ...
            val goTo = ::gen_goTo_fn
            return fun(): Any? {
                val _component_up_icon = resolveEasyComponent("up-icon", GenUniModulesUviewUltraComponentsUpIconUpIconClass)
                return if (isTrue(shouldShow.value)) {
                    _cE("view", _uM("key" to 0, "class" to "weapp-tw-border up-pagination"), _uA(
                        _cE("view", _uM("class" to _nC(_uA(
                            "weapp-tw-border up-pagination-btn",
                            _uM("btn-disabled" to (_ctx.currentPage <= 1))
                        )), "style" to _nS(_uM("backgroundColor" to _ctx.buttonBgColor, "borderColor" to _ctx.buttonBorderColor)), "onClick" to prev), _uA(
                            if (_ctx.prevText != "") {
                                _cE("text", _uM("key" to 0, "class" to "weapp-tw-border up-pagination-text"), _tD(_ctx.prevText), 1)
                            } else {
                                _cV(_component_up_icon, _uM("key" to 1, "name" to "arrow-left", "size" to "14"))
                            }
                        ), 6),
                        if (isTrue(showPager.value)) {
                            _cE(Fragment, _uM("key" to 0), RenderHelpers.renderList(displayedPages.value, fun(page, index, __index, _cached): Any {
                                return _cE("view", _uM("key" to index, "class" to _nC(_uA(
                                    "weapp-tw-border up-pagination-item",
                                    _uM("item-active" to (page == currentPageStr.value))
                                )), "onClick" to fun(){
                                    goTo(page)
                                }), _uA(
                                    _cE("text", _uM("class" to _nC(_uA(
                                        "weapp-tw-border up-pagination-text",
                                        _uM("text-active" to (page == currentPageStr.value))
                                    ))), _tD(page), 3)
                                ), 10, _uA(
                                    "onClick"
                                ))
                        ...
                        if (isTrue(showTotal.value)) {
                            _cE("view", _uM("key" to 1, "class" to "weapp-tw-border up-pagination-total"), _uA(
                                _cE("text", _uM("class" to "weapp-tw-border up-pagination-text"), "共 " + _tD(_ctx.total) + " 条", 1)
                            ))
                        } else {
                            _cC("v-if", true)
                        },
                        _cE("view", _uM("class" to _nC(_uA(
                            "weapp-tw-border up-pagination-btn",
                            _uM("btn-disabled" to (_ctx.currentPage >= totalPages.value))
                        )), "style" to _nS(_uM("backgroundColor" to _ctx.buttonBgColor, "borderColor" to _ctx.buttonBorderColor)), "onClick" to next), _uA(
                            if (_ctx.nextText != "") {
                                _cE("text", _uM("key" to 0, "class" to "weapp-tw-border up-pagination-text"), _tD(_ctx.nextText), 1)
                            } else {
