    ...
    companion object {
        ...
        var setup: (__props: GenUniModulesZPagingXComponentsZPagingXComponentsZPagingLoadMore) -> Any? = fun(__props): Any? {
            ...
            val isDefault = computed(fun(): Boolean {
                return props.status === default__17.More.Default
            }
            )
            val isLoading = computed(fun(): Boolean {
                return props.status === default__17.More.Loading
            }
            )
            val isNoMore = computed(fun(): Boolean {
                return props.status === default__17.More.NoMore
            }
            )
            val isFail = computed(fun(): Boolean {
                return props.status === default__17.More.Fail
            }
            )
            ...
            return fun(): Any? {
                return _cE("view", _uM("class" to "weapp-tw-border zpx-l-container", "onClick" to onClickMore), _uA(
                    if (isTrue(isNoMore.value)) {
                        _cE("view", _uM("key" to 0, "class" to "weapp-tw-border zpx-l-line"))
                    } else {
                        _cC("v-if", true)
                    }
                    ,
                    ...
                    ,
                    _cE("text", _uM("class" to "weapp-tw-border zpx-l-loading-text"), _tD(statusText.value), 1),
                    if (isTrue(isNoMore.value)) {
                        _cE("view", _uM("key" to 2, "class" to "weapp-tw-border zpx-l-line"))
                    } else {
                        _cC("v-if", true)
                    }
                ))
