...
open class GenUniModulesZPagingXComponentsZPagingXZPagingX : VueComponent {
    ...
    companion object {
        ...
        var setup: (__props: GenUniModulesZPagingXComponentsZPagingXZPagingX, __setupCtx: SetupContext) -> Any? = fun(__props, __setupCtx): Any? {
            ...
            val refresherStatus = ref(default__20.Refresher.Default)
            ...
            val loadMoreStatus = ref(default__20.More.Default)
            val queryFrom = ref("")
            ...
            val reload = fun(){
                ...
                queryFrom.value = default__20.QueryFrom.Reload
            ...
            val endRefresh = fun(){
                ...
                refresherStatus.value = default__20.Refresher.Default
            ...
            val complete = fun(data: UTSArray<Any>?){
                ...
                val needCompleteDelay = props.refresherCompleteDelay > 0 && (queryFrom.value === default__20.QueryFrom.UserPullDown || props.showRefresherWhenReload)
                if (needCompleteDelay) {
                    refresherStatus.value = default__20.Refresher.Complete
                    _cleanRefresherCompleteDelayTimer()
                    _cleanRefresherEndAnimateTimer()
                ...
                if (customNoMore.value != -1) {
                    if (customNoMore.value == 1 || (customNoMore.value != 0 && finalData.length == 0)) {
                        loadMoreStatus.value = default__20.More.NoMore
                    }
                } else {
                    if (finalData.length == 0 || (finalData.length > 0 && finalData.length < props.defaultPageSize)) {
                        loadMoreStatus.value = default__20.More.NoMore
                    } else {
                        loadMoreStatus.value = default__20.More.Default
                    }
                }
            ...
            val refresh = fun(){
                ...
                isLoading.value = true
                pageNo.value = props.defaultPageNo
                queryFrom.value = default__20.QueryFrom.Refresh
            ...
            val _onRefresherpulling = fun(e: RefresherEvent){
                ...
                if (!isLoading.value) {
                    refresherStatus.value = if (e.detail.dy < finalRefresherThreshold.value) {
                        default__20.Refresher.Default
                    } else {
                        default__20.Refresher.ReleaseToRefresh
                    }
                }
            }
            val _handleRefresh = fun(){
                _cleanRefresherCompleteDelayTimer()
                _cleanRefresherEndAnimateTimer()
                refresherStatus.value = default__20.Refresher.Loading
                refresherTriggered.value = true
                pageNo.value = props.defaultPageNo
                queryFrom.value = default__20.QueryFrom.UserPullDown
                _emitQuery(pageNo.value, props.defaultPageSize)
            }
            ...
            val _onRefresherrestore = fun(e: RefresherEvent){
                ...
                if (refresherStatus.value !== default__20.Refresher.Complete) {
                    refresherStatus.value = default__20.Refresher.Default
                }
            }
            ...
            val _handleLoadMore = fun(){
                if (!props.loadMoreEnabled || isLoading.value || loadMoreStatus.value === default__20.More.NoMore) {
                    return
                }
                loadMoreStatus.value = default__20.More.Loading
                pageNo.value++
                queryFrom.value = default__20.QueryFrom.LoadMore
                _emitQuery(pageNo.value, props.defaultPageSize)
            }
