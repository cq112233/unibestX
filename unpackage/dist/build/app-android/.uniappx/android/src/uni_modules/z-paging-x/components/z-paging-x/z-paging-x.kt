...
open class GenUniModulesZPagingXComponentsZPagingXZPagingX : VueComponent {
    ...
    companion object {
        ...
        var setup: (__props: GenUniModulesZPagingXComponentsZPagingXZPagingX, __setupCtx: SetupContext) -> Any? = fun(__props, __setupCtx): Any? {
            ...
            val refresherStatus = ref(default__17.Refresher.Default)
            ...
            val loadMoreStatus = ref(default__17.More.Default)
            val queryFrom = ref("")
            ...
            val reload = fun(){
                ...
                queryFrom.value = default__17.QueryFrom.Reload
            ...
            val endRefresh = fun(){
                ...
                refresherStatus.value = default__17.Refresher.Default
            ...
            val complete = fun(data: UTSArray<Any>?){
                ...
                val needCompleteDelay = props.refresherCompleteDelay > 0 && (queryFrom.value === default__17.QueryFrom.UserPullDown || props.showRefresherWhenReload)
                if (needCompleteDelay) {
                    refresherStatus.value = default__17.Refresher.Complete
                    _cleanRefresherCompleteDelayTimer()
                    _cleanRefresherEndAnimateTimer()
                ...
                if (customNoMore.value != -1) {
                    if (customNoMore.value == 1 || (customNoMore.value != 0 && finalData.length == 0)) {
                        loadMoreStatus.value = default__17.More.NoMore
                    }
                } else {
                    if (finalData.length == 0 || (finalData.length > 0 && finalData.length < props.defaultPageSize)) {
                        loadMoreStatus.value = default__17.More.NoMore
                    } else {
                        loadMoreStatus.value = default__17.More.Default
                    }
                }
            ...
            val refresh = fun(){
                ...
                isLoading.value = true
                pageNo.value = props.defaultPageNo
                queryFrom.value = default__17.QueryFrom.Refresh
            ...
            val _onRefresherpulling = fun(e: RefresherEvent){
                ...
                if (!isLoading.value) {
                    refresherStatus.value = if (e.detail.dy < finalRefresherThreshold.value) {
                        default__17.Refresher.Default
                    } else {
                        default__17.Refresher.ReleaseToRefresh
                    }
                }
            }
            val _handleRefresh = fun(){
                _cleanRefresherCompleteDelayTimer()
                _cleanRefresherEndAnimateTimer()
                refresherStatus.value = default__17.Refresher.Loading
                refresherTriggered.value = true
                pageNo.value = props.defaultPageNo
                queryFrom.value = default__17.QueryFrom.UserPullDown
                _emitQuery(pageNo.value, props.defaultPageSize)
            }
            ...
            val _onRefresherrestore = fun(e: RefresherEvent){
                ...
                if (refresherStatus.value !== default__17.Refresher.Complete) {
                    refresherStatus.value = default__17.Refresher.Default
                }
            }
            ...
            val _handleLoadMore = fun(){
                if (!props.loadMoreEnabled || isLoading.value || loadMoreStatus.value === default__17.More.NoMore) {
                    return
                }
                loadMoreStatus.value = default__17.More.Loading
                pageNo.value++
                queryFrom.value = default__17.QueryFrom.LoadMore
                _emitQuery(pageNo.value, props.defaultPageSize)
            }
            ...
            return fun(): Any? {
                ...
                return _cE("view", _uM("class" to _nC(_uA(
                    "weapp-tw-border",
                    _uM("zpx-content" to true)
                )), "ref_key" to "zPagingX", "ref" to zPagingX, "style" to _nS(_uA(
                    _ctx.pagingStyle
                ))), _uA(
                    ...
                    if (_ctx.listIs === "list-view") {
                        _cE("list-view", _uM("key" to 0, "ref_key" to "zpxList", "ref" to zpxList, "class" to "zpx-list-view zpx-flex1", "id" to _ctx.listId, "refresher-enabled" to _ctx.refresherEnabled, "refresher-threshold" to finalRefresherThreshold.value, "refresher-triggered" to refresherTriggered.value, "refresher-default-style" to if (isTruthy(_ctx.useCustomRefresher)) {
                            ...
                            _cE("list-item", _uM("slot" to "refresher"), _uA(
                                _cE("view", _uM("ref_key" to "zpxListRefresher", "ref" to zpxListRefresher, "class" to "weapp-tw-border zpx-list-refresher", "style" to _nS(_uM("width" to (contentWidth.value + "px")))), _uA(
                        ...
                        _cE("scroll-view", _uM("key" to 1, "ref_key" to "zpxList", "ref" to zpxList, "class" to "zpx-list-view zpx-flex1", "id" to _ctx.listId, "direction" to "vertical", "refresher-enabled" to _ctx.refresherEnabled, "refresher-threshold" to finalRefresherThreshold.value, "refresher-triggered" to refresherTriggered.value, "refresher-default-style" to if (isTruthy(_ctx.useCustomRefresher)) {
                            "none"
                        } else {
                            "black"
                        }
                        , "refresher-background" to _ctx.refresherBackground, "scroll-top" to scrollTop.value, "show-scrollbar" to _ctx.showScrollbar, "scroll-with-animation" to finalScrollWithAnimation.value, "onRefresherpulling" to _onRefresherpulling, "onRefresherrefresh" to _onRefresherrefresh, "onRefresherrestore" to _onRefresherrestore, "onRefresherabort" to _onRefresherabort, "onScrolltolower" to _onScrolltolower, "onScrolltoupper" to _onScrolltoupper, "onScroll" to _onScroll, "onStartnestedscroll" to _onStartNestedScroll, "onNestedprescroll" to _onNestedPreScroll), _uA(
                            renderSlot(_ctx.`$slots`, "default"),
                            ...
                            _cE("view", _uM("class" to "weapp-tw-border", "slot" to "refresher"), _uA(
                                _cE("view", _uM("ref_key" to "zpxListRefresher", "ref" to zpxListRefresher, "class" to "weapp-tw-border zpx-list-refresher", "style" to _nS(_uM("width" to (contentWidth.value + "px")))), _uA(
                                    if (isTrue(_ctx.`$slots`["refresher"])) {
                                        renderSlot(_ctx.`$slots`, "refresher", _uM("key" to 0, "refresherStatus" to refresherStatus.value))
                                    } else {
