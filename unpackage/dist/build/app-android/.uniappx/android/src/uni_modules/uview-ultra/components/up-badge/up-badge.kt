...
open class GenUniModulesUviewUltraComponentsUpBadgeUpBadge : VueComponent {
    ...
    companion object {
        ...
        var setup: (__props: GenUniModulesUviewUltraComponentsUpBadgeUpBadge) -> Any? = fun(__props): Any? {
            ...
            val props = __props
            val badgeStyle = computed<UTSJSONObject>(fun(): UTSJSONObject {
            ...
            val computedBadgeStyle = computed(fun(): UTSJSONObject {
                val custom = addStyle(props.customStyle) as UTSJSONObject
                val badge = badgeStyle.value
                return deepMerge(badge, custom) as UTSJSONObject
            }
            )
            val showValue = computed<String>(fun(): String {
                var valueReturn = ""
                val valStr = if (props.value != "") {
                ...
                when (props.numberType.toString()) {
                    "overflow" -> 
                        valueReturn = if (parseInt(valStr) > parseInt(props.max.toString())) {
                            props.max.toString() + "+"
                        } else {
                            valStr
                        }
                    ...
                    "limit" -> 
                        valueReturn = if (parseInt(valStr) > 999) {
                            if (parseInt(valStr) >= 9999) {
                                (Math.floor(parseInt(valStr) / 1e4 * 100) / 100).toString(10) + "w"
                            } else {
                                (Math.floor(parseInt(valStr) / 1e3 * 100) / 100).toString(10) + "k"
                            }
                        } else {
                            valStr
                        }
            ...
            return fun(): Any? {
                return if (isTrue(_ctx.show && ((if (parseInt(_ctx.value.toString()) == 0) {
                    ...
                    )), "style" to _nS(computedBadgeStyle.value)), _tD(if (_ctx.isDot) {
                        ""
                    } else {
                        showValue.value
                    }), 7)
        ...
        var props = _nP(_uM("customClass" to _uM("type" to "String", "default" to ""), "url" to _uM("type" to "String", "default" to ""), "linkType" to _uM("type" to "String", "default" to "navigateTo"), "isDot" to _uM("type" to "Boolean", "default" to false), "value" to _uM("type" to _uA(
            "Number",
            "String"
        ), "default" to ""), "modelValue" to _uM("type" to _uA(
            "Number",
            "String"
        ), "default" to ""), "show" to _uM("type" to "Boolean", "default" to true), "max" to _uM("type" to _uA(
            "Number",
            "String"
        ), "default" to 999), "type" to _uM("type" to "String", "default" to "error"), "showZero" to _uM("type" to "Boolean", "default" to false), "bgColor" to _uM("type" to "String", "default" to ""), "color" to _uM("type" to "String", "default" to "#ffffff"), "shape" to _uM("type" to "String", "default" to "circle"), "numberType" to _uM("type" to "String", "default" to "overflow"), "offset" to _uM("type" to "Array", "default" to fun(): UTSArray<Any> {
            return _uA<Any>()
        }
        ), "inverted" to _uM("type" to "Boolean", "default" to false), "absolute" to _uM("type" to "Boolean", "default" to false), "customStyle" to _uM("type" to _uA(
            "Object",
            "String"
