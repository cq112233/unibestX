...
open class GenUniModulesMpHtmlComponentsMpHtmlMpHtml : VueComponent {
    ...
    open var setContent: (html: String) -> Unit
        get() {
            return unref(this.`$exposed`["setContent"]) as (html: String) -> Unit
        }
        set(value) {
            setRefValue(this.`$exposed`, "setContent", value)
        }
    open var getText: () -> String
        get() {
            return unref(this.`$exposed`["getText"]) as () -> String
        }
        set(value) {
            setRefValue(this.`$exposed`, "getText", value)
        }
    ...
    companion object {
        ...
        var setup: (__props: GenUniModulesMpHtmlComponentsMpHtmlMpHtml, __setupCtx: SetupContext) -> Any? = fun(__props, __setupCtx): Any? {
            val __expose = __setupCtx.expose
            val __ins = getCurrentInstance()!!
            val _ctx = __ins.proxy as GenUniModulesMpHtmlComponentsMpHtmlMpHtml
            val _cache = __ins.renderCache
            val props = __props
            fun emit(event: String, vararg do_not_transform_spread: Any?) {
                __ins.emit(event, *do_not_transform_spread)
            }
            val localNodes = ref(_uA<HtmlNode>())
            fun gen_isSpace_fn(ch: String): Boolean {
                return ch == " " || ch == "\t" || ch == "\n" || ch == "\r"
            }
            val isSpace = ::gen_isSpace_fn
            fun gen_isInline_fn(name: String): Boolean {
            ...
            val isInline = ::gen_isInline_fn
            fun gen_isHeading_fn(name: String): Boolean {
                return name.length == 2 && name.charAt(0) == "h" && name.charAt(1) >= "1" && name.charAt(1) <= "6"
            }
            val isHeading = ::gen_isHeading_fn
            fun gen_cleanText_fn(s: String): String {
                var r = ""
                var lastSpace = false
                run {
                    var i: Number = 0
            ...
            val cleanText = ::gen_cleanText_fn
            fun gen_stripAllTags_fn(s: String): String {
                var r = ""
                var inTag = false
                run {
                    var i: Number = 0
            ...
            val stripAllTags = ::gen_stripAllTags_fn
            fun gen_flatText_fn(n: HtmlNode): String {
                if (n.type == "text") {
                    return n.text ?: ""
                }
                if (n.children == null) {
            ...
            val flatText = ::gen_flatText_fn
            fun gen_inlineStyle_fn(name: String): String {
            ...
            val inlineStyle = ::gen_inlineStyle_fn
            fun gen_headingStyle_fn(name: String): String {
            ...
            val headingStyle = ::gen_headingStyle_fn
            fun gen_parseAttrs_fn(tag: String): UTSJSONObject {
            ...
            val parseAttrs = ::gen_parseAttrs_fn
            fun gen_findMatchingClose_fn(html: String, start: Number, tagName: String): Number {
                var depth: Number = 1
                var i = start
                val openStr = "<" + tagName
            ...
            val findMatchingClose = ::gen_findMatchingClose_fn
            fun gen_parseTable_fn(html: String): UTSArray<HtmlNode> {
                ...
                while(i < html.length){
                    ...
                    while(ci < trInner.length){
                        ...
                        val text = stripAllTags(trInner.substring(tdGt + 1, tdE))
            ...
            val parseTable = ::gen_parseTable_fn
            fun gen_parseList_fn(html: String): UTSArray<HtmlNode> {
                ...
                while(true){
                    ...
                    val text = stripAllTags(html.substring(liGt + 1, liE))
                    if (text.length > 0) {
            ...
            val parseList = ::gen_parseList_fn
            fun gen_parseHtml_fn(html: String): UTSArray<HtmlNode> {
                ...
                while(i < len){
                    ...
                    val closeStr = "</" + name + ">"
                    val closePos = findMatchingClose(html, i, name)
                    if (closePos == -1) {
                        continue
                    }
                    ...
                    if (name == "table") {
                        ...
                        node.children = _uA(
                            HtmlNode(type = "text", text = stripAllTags(inner))
                        )
                        ...
                        node.children = gen_parseHtml_fn(inner)
                        node.name = "div"
                    } else {
                        node.children = gen_parseHtml_fn(inner)
                        node.name = "div"
            ...
            val parseHtml = ::gen_parseHtml_fn
            fun gen_doParse_fn(html: String): Unit {
                val nodes = parseHtml(html)
            ...
            val doParse = ::gen_doParse_fn
            fun gen_setContent_fn(html: String): Unit {
                doParse(html)
            }
            val setContent = ::gen_setContent_fn
            fun gen_getText_fn(): String {
                return flatText(HtmlNode(children = localNodes.value))
            }
            val getText = ::gen_getText_fn
            fun gen_onLinkTap_fn(n: HtmlNode): Unit {
                val href = if (n.attrs != null) {
                    ((n.attrs!!["href"] as String) ?: "")
                } else {
                    ""
                }
                emit("linktap", n.attrs ?: (_uO()))
            ...
            val onLinkTap = ::gen_onLinkTap_fn
            watch(fun(): String {
            ...
            __expose(_uM("setContent" to setContent, "getText" to getText))
            return fun(): Any? {
                return _cE("view", _uM("class" to "weapp-tw-border _mp_root"), _uA(
                    _cE(Fragment, null, RenderHelpers.renderList(localNodes.value, fun(n, i, __index, _cached): Any {
                        return _cE(Fragment, _uM("key" to i), _uA(
                            if (n.type == "text") {
                                _cE("text", _uM("key" to 0, "class" to "weapp-tw-border"), _tD(n.text), 1)
                            } else {
                                if (n.name == "br") {
                                    _cE("text", _uM("key" to 1, "class" to "weapp-tw-border"), "\n")
                                } else {
                                    if (n.name == "img") {
                                        ...
                                        if (n.name == "a") {
                                            _cE("text", _uM("key" to 3, "class" to "weapp-tw-border mp-a", "onClick" to fun(){
                                                onLinkTap(n)
                                            }), _tD(flatText(n)), 9, _uA(
                                                "onClick"
                                            ))
                                        } else {
                                            if (isTrue(isInline(n.name ?: ""))) {
                                                _cE("text", _uM("key" to 4, "class" to "weapp-tw-border", "style" to _nS(inlineStyle(n.name ?: ""))), _tD(flatText(n)), 5)
                                            } else {
                                                if (isTrue(n.name == "h1" || n.name == "h2" || n.name == "h3" || n.name == "h4" || n.name == "h5" || n.name == "h6")) {
                                                    _cE("text", _uM("key" to 5, "class" to "weapp-tw-border", "style" to _nS(headingStyle(n.name ?: ""))), _tD(flatText(n)), 5)
                                                } else {
                                                    if (isTrue(n.name == "pre" || n.name == "code")) {
                                                        _cE("text", _uM("key" to 6, "class" to "weapp-tw-border", "style" to _nS(_uM("font-family" to "monospace", "white-space" to "pre-wrap"))), _tD(flatText(n)), 5)
                                                    } else {
                                                        if (n.name == "hr") {
                                                            _cE("view", _uM("key" to 7, "class" to "weapp-tw-border", "style" to _nS(_uM("height" to "1px", "background-color" to "#e0e0e0", "margin" to "10px 0"))), null, 4)
                                                        } else {
                                                            if (n.name == "table") {
                                                                _cE("view", _uM("key" to 8, "class" to "weapp-tw-border", "style" to _nS(_uM("display" to "flex", "flex-direction" to "column"))), _uA(
                                                                    _cE(Fragment, null, RenderHelpers.renderList((n.children ?: _uA()), fun(row, ri, __index, _cached): Any {
                                                                        return _cE("view", _uM("class" to "weapp-tw-border", "key" to ri, "style" to _nS(_uM("display" to "flex", "flex-direction" to "row"))), _uA(
                                                                            _cE(Fragment, null, RenderHelpers.renderList((row.children ?: _uA()), fun(cell, ci, __index, _cached): Any {
                                                                                return _cE("view", _uM("class" to "weapp-tw-border", "key" to ci, "style" to _nS((if (cell.name == "th") {
                                                                                    "background-color:#f5f5f5;"
                                                                                } else {
                                                                                    ""
                                                                                }) + "flex:1;padding:4px 8px;border-width:1px;border-color:#d0d0d0;border-style:solid")), _uA(
                                                                                    _cE("text", _uM("class" to "weapp-tw-border", "style" to _nS(if (cell.name == "th") {
                                                                                        "font-weight:bold;"
                                                                                    } else {
                                                                                        ""
                                                                                    })), _tD(flatText(cell)), 5)
                                                                ...
                                                                if (isTrue(n.name == "ul" || n.name == "ol")) {
                                                                    _cE("view", _uM("key" to 9, "class" to "weapp-tw-border", "style" to _nS(_uM("padding-left" to "20px"))), _uA(
                                                                        _cE(Fragment, null, RenderHelpers.renderList((n.children ?: _uA()), fun(li, lii, __index, _cached): Any {
                                                                            return _cE("view", _uM("class" to "weapp-tw-border", "key" to lii, "style" to _nS(_uM("flex-direction" to "row"))), _uA(
                                                                                _cE("text", _uM("class" to "weapp-tw-border"), _tD(if (n.name == "ol") {
                                                                                    (lii + 1).toString(10) + ". "
                                                                                } else {
                                                                                    "• "
                                                                                }), 1),
                                                                                _cE("text", _uM("class" to "weapp-tw-border"), _tD(flatText(li)), 1)
                                                                            ), 4)
                                                                        }), 128)
                                                                    ), 4)
                                                                } else {
                                                                    _cE("view", _uM("key" to 10, "class" to "weapp-tw-border"), _uA(
                                                                        _cE(Fragment, null, RenderHelpers.renderList((n.children ?: _uA()), fun(cn, ci, __index, _cached): Any {
                                                                            return _cE(Fragment, _uM("key" to ci), _uA(
                                                                                if (cn.type == "text") {
                                                                                    _cE("text", _uM("key" to 0, "class" to "weapp-tw-border"), _tD(cn.text), 1)
                                                                                } else {
                                                                                    if (cn.name == "br") {
                                                                                        _cE("text", _uM("key" to 1, "class" to "weapp-tw-border"), "\n")
                                                                                    } else {
                                                                                        if (cn.name == "img") {
                                                                                            ...
                                                                                            if (cn.name == "a") {
                                                                                                _cE("text", _uM("key" to 3, "class" to "weapp-tw-border mp-a", "onClick" to fun(){
                                                                                                    onLinkTap(cn)
                                                                                                }), _tD(flatText(cn)), 9, _uA(
                                                                                                    "onClick"
                                                                                                ))
                                                                                            } else {
                                                                                                if (isTrue(isInline(cn.name ?: ""))) {
                                                                                                    _cE("text", _uM("key" to 4, "class" to "weapp-tw-border", "style" to _nS(inlineStyle(cn.name ?: ""))), _tD(flatText(cn)), 5)
                                                                                                } else {
                                                                                                    _cE("text", _uM("key" to 5, "class" to "weapp-tw-border"), _tD(flatText(cn)), 1)
                                                                                                }
                                                                                            }
                                                                                        }
                                                                                    }
        ...
        val styles0: Map<String, Map<String, Map<String, Any>>>
            get() {
                return _uM("weapp-tw-border" to _pS(_uM("borderTopWidth" to 0, "borderRightWidth" to 0, "borderBottomWidth" to 0, "borderLeftWidth" to 0)), "_mp_root" to _pS(_uM("paddingTop" to 1, "paddingRight" to 0, "paddingBottom" to 1, "paddingLeft" to 0)), "mp-a" to _pS(_uM("color" to "#366092")))
            }
