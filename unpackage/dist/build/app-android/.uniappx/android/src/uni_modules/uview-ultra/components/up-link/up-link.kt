...
open class GenUniModulesUviewUltraComponentsUpLinkUpLink : VueComponent {
    ...
    companion object {
        ...
        var setup: (__props: GenUniModulesUviewUltraComponentsUpLinkUpLink) -> Any? = fun(__props): Any? {
            val __ins = getCurrentInstance()!!
            val _ctx = __ins.proxy as GenUniModulesUviewUltraComponentsUpLinkUpLink
            val _cache = __ins.renderCache
            val props = __props
            fun emit(event: String, vararg do_not_transform_spread: Any?) {
                __ins.emit(event, *do_not_transform_spread)
            }
            val linkStyle = computed<UTSJSONObject>(fun(): UTSJSONObject {
                val style: UTSJSONObject = _uO("color" to props.color, "fontSize" to addUnit(props.fontSize), "lineHeight" to addUnit(parseInt(getPx(props.fontSize)) + 2), "textDecoration" to if (props.underLine) {
                    "underline"
                } else {
                    "none"
                }
            ...
            val mergedLinkStyle = computed(fun(): UTSJSONObject {
                val custom = addStyle(props.customStyle) as UTSJSONObject
                val link = linkStyle.value
                return deepMerge(link, custom) as UTSJSONObject
            }
            )
            fun gen_openLink_fn(): Unit {
                emit("click")
            }
            val openLink = ::gen_openLink_fn
            return fun(): Any? {
                return _cE("text", _uM("class" to "weapp-tw-border up-link", "onClick" to withModifiers(openLink, _uA(
                    "stop"
                )), "style" to _nS(mergedLinkStyle.value)), _tD(_ctx.text), 5)
            }
        }
        var name = "up-link"
        ...
        var props = _nP(_uM("customClass" to _uM("type" to "String", "default" to ""), "url" to _uM("type" to "String", "default" to ""), "linkType" to _uM("type" to "String", "default" to "navigateTo"), "color" to _uM("type" to "String", "default" to "#3c9cff"), "fontSize" to _uM("type" to _uA(
            "String",
            "Number"
        ), "default" to "15px"), "underLine" to _uM("type" to "Boolean", "default" to false), "href" to _uM("type" to "String", "default" to ""), "mpTips" to _uM("type" to "String", "default" to "链接已复制，请在浏览器打开"), "lineColor" to _uM("type" to "String", "default" to ""), "text" to _uM("type" to "String", "default" to ""), "customStyle" to _uM("default" to fun(): UTSJSONObject {
            return (_uO())
        }
        )))
