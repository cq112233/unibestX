    ...
    companion object {
        ...
        var setup: (__props: GenUniModulesZPagingXComponentsZPagingXComponentsZPagingRefresher) -> Any? = fun(__props): Any? {
            ...
            val isDefault = computed(fun(): Boolean {
                return props.status === default__17.Refresher.Default
            }
            )
            val isReleaseToRefresh = computed(fun(): Boolean {
                return props.status === default__17.Refresher.ReleaseToRefresh
            }
            )
            val isLoading = computed(fun(): Boolean {
                return props.status === default__17.Refresher.Loading
            }
            )
            val isComplete = computed(fun(): Boolean {
                return props.status === default__17.Refresher.Complete
            }
            )
            ...
            return fun(): Any? {
                return _cE("view", _uM("class" to "weapp-tw-border zpx-r-container", "style" to _nS(_uM("height" to if (_ctx.showUpdateTime) {
                    "60px"
                } else {
                    "40px"
                }
                    ...
                    ,
                    _cE("view", _uM("class" to "weapp-tw-border zpx-r-text-container", "style" to _nS(_uM("marginLeft" to if (showRefresherTimeText.value) {
                        "25rpx"
                    } else {
                        "10rpx"
                    }
                    ))), _uA(
                        _cE("text", _uM("class" to "weapp-tw-border zpx-r-text"), _tD(statusText.value), 1),
                        if (isTrue(showRefresherTimeText.value)) {
                            _cE("text", _uM("key" to 0, "class" to "weapp-tw-border zpx-r-text zpx-r-time-text"), _tD(refresherTimeText.value), 1)
                        } else {
                            _cC("v-if", true)
                        }
                    ), 4)
