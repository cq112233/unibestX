                ...
                return _cV(unref(GenSrcPagesBasicComponentsCardClass), _uM("title" to "路由拦截与传参测试"), _uM("default" to withSlotCtx(fun(): UTSArray<Any> {
                    return _uA(
                        ...
                        _cE("view", _uM("class" to "flex-row justify-between"), _uA(
                            _cE("button", _uM("class" to "flex-1 bg-__3182ce_ text-white rounded-8px text-14px h-42px", "onClick" to jumpToTest), " 跳转 Test 页 (传参) "),
                            _cE("button", _uM("class" to "flex-1 ml-12px bg-__ef4444_ text-white rounded-8px text-14px h-42px", "onClick" to handleLogout), " 清除登录状态 ")
                        ))
                    )
                }
                ), "_" to 1))
