...
open class GenUniModulesMpHtmlComponentsMpHtmlMpHtml : VueComponent {
    ...
    open fun gen_parseAttrs_fn(tag: String): UTSJSONObject {
        val attrs: UTSJSONObject = _uO("__\$originalPosition" to UTSSourceMapPosition("attrs", "uni_modules/mp-html/components/mp-html/mp-html.uvue", 226, 13))
        var i: Number = 0
    ...
    open fun gen_parseTable_fn(html: String): UTSArray<HtmlNode> {
        ...
        while(true){
            ...
            var ci: Number = 0
            while(true){
                val tdS = html.indexOf("<t", trGt + 1 + ci)
                if (tdS == -1 || tdS >= trE) {
                    break
                }
                val cellTag = if (html.charAt(tdS + 1) == "h") {
                    "th"
                } else {
                    if (html.charAt(tdS + 1) == "d") {
                        "td"
                    } else {
                        ""
                    }
                ...
                val tdGt = html.indexOf(">", tdS)
                if (tdGt == -1 || tdGt >= trE) {
                    break
                }
                val tdE = html.indexOf("</" + cellTag + ">", tdGt)
                if (tdE == -1 || tdE >= trE) {
                    break
                }
                val text = this.stripAllTags(html.substring(tdGt + 1, tdE))
    ...
    open fun gen_headingStyle_fn(name: String): String {
        val sz: UTSJSONObject = _uO("__\$originalPosition" to UTSSourceMapPosition("sz", "uni_modules/mp-html/components/mp-html/mp-html.uvue", 383, 13), "h1" to "2em", "h2" to "1.5em", "h3" to "1.17em", "h4" to "1em", "h5" to "0.83em", "h6" to "0.67em")
    ...
    open fun gen_onLinkTap_fn(n: HtmlNode): Unit {
        ...
        if (href.length > 0 && href.includes("://")) {
            uni_setClipboardData(SetClipboardDataOptions(data = href, success = fun(_){
                return uni_showToast(ShowToastOptions(title = "链接已复制", icon = "none"))
            }
            ))
        }
    }
