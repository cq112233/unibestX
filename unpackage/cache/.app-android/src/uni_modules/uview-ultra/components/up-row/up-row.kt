...
open class GenUniModulesUviewUltraComponentsUpRowUpRow : VueComponent {
    ...
    open var getComponentWidth: () -> UTSPromise<Number?>
        get() {
            return unref(this.`$exposed`["getComponentWidth"]) as () -> UTSPromise<Number?>
        }
        set(value) {
            setRefValue(this.`$exposed`, "getComponentWidth", value)
        }
    ...
    companion object {
        ...
        var setup: (__props: GenUniModulesUviewUltraComponentsUpRowUpRow, __setupCtx: SetupContext) -> Any? = fun(__props, __setupCtx): Any? {
            val __expose = __setupCtx.expose
            val __ins = getCurrentInstance()!!
            val _ctx = __ins.proxy as GenUniModulesUviewUltraComponentsUpRowUpRow
            val _cache = __ins.renderCache
            val children = useUltraUI().children
            val props = __props
            ...
            val rowStyle = computed(fun(): Any {
                val style: UTSJSONObject = _uO("__\$originalPosition" to UTSSourceMapPosition("style", "uni_modules/uview-ultra/components/up-row/up-row.uvue", 60, 9), "alignItems" to uAlignItem.value, "justifyContent" to uJustify.value)
                if (props.gutter != "") {
            ...
            val getComponentWidth = fun(): UTSPromise<Number?> {
                return UTSPromise(fun(resolve, _reject){
                    sleep().then(fun(){
                        uni.UNI5198058.upGetRect(".up-row").then(fun(res: Any){
                            var node = res as NodeInfo
                            resolve(if (node.width != null) {
                                node.width!!
                            } else {
                                0
                            }
            ...
            val getProps = fun(): UTSJSONObject {
                return _uO("gutter" to props.gutter, "justify" to props.justify, "align" to props.align)
            }
            val getRefs = fun(): UTSJSONObject {
                return _uO()
            }
            __expose(_uM("getComponentWidth" to getComponentWidth, "getProps" to getProps, "getRefs" to getRefs))
            return fun(): Any? {
                return _cE("view", _uM("class" to "up-row", "ref" to "up-row", "style" to _nS(_uA(
                    rowStyle.value,
                    _ctx.`$upAddStyle`(props.customStyle)
