...
open class GenUniModulesUviewUltraComponentsUpCountDownUpCountDown : VueComponent {
    ...
    companion object {
        ...
        var setup: (__props: GenUniModulesUviewUltraComponentsUpCountDownUpCountDown, __setupCtx: SetupContext) -> Any? = fun(__props, __setupCtx): Any? {
            val __expose = __setupCtx.expose
            val __ins = getCurrentInstance()!!
            val _ctx = __ins.proxy as GenUniModulesUviewUltraComponentsUpCountDownUpCountDown
            ...
            val timer = ref(-1)
            val timeData = ref(parseTimeData(0))
            val formattedTime = ref("0")
            val running = ref(false)
            val endTime = ref(0)
            val remainTime = ref(0)
            val clearTimeoutFn = fun(){
                if (timer.value != -1) {
            ...
            val setRemainTime = fun(remain: Number){
                ...
                val timeDataValue = parseTimeData(remain)
            ...
            val macroTick = ::gen_macroTick_fn
            fun gen_microTick_fn() {
                clearTimeoutFn()
                timer.value = setTimeout(fun(){
            ...
            val microTick = ::gen_microTick_fn
            val toTick = fun(){
            ...
            __expose(_uM("start" to start, "pause" to pause, "reset" to reset))
            return fun(): Any? {
                return _cE("view", _uM("class" to "up-count-down"), _uA(
                    renderSlot(_ctx.`$slots`, "default", _uO(), fun(): UTSArray<Any> {
                        return _uA(
                            _cE("text", _uM("class" to "up-count-down__text"), _tD(formattedTime.value), 1)
                        )
                    }
                    )
                ))
