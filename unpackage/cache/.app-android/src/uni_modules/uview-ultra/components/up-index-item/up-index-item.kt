...
open class GenUniModulesUviewUltraComponentsUpIndexItemUpIndexItem : VueComponent {
    ...
    open var setId: (idPam: String) -> Unit
        get() {
            return unref(this.`$exposed`["setId"]) as (idPam: String) -> Unit
        }
        set(value) {
            setRefValue(this.`$exposed`, "setId", value)
        }
    ...
    companion object {
        ...
        var setup: (__props: GenUniModulesUviewUltraComponentsUpIndexItemUpIndexItem, __setupCtx: SetupContext) -> Any? = fun(__props, __setupCtx): Any? {
            val __expose = __setupCtx.expose
            val __ins = getCurrentInstance()!!
            val _ctx = __ins.proxy as GenUniModulesUviewUltraComponentsUpIndexItemUpIndexItem
            ...
            val itemId = ref("")
            val anchor = ref(_uO())
            val getIndexItemRect = fun(): UTSPromise<NodeInfo> {
                return UTSPromise(fun(resolve, _reject){
                    uni.UNI5198058.upGetRect(".up-index-item", false, instance).then(fun(size: Any){
            ...
            val init = fun() {
                ...
                sleep().then(fun(){
                    getIndexItemRect().then(fun(size: NodeInfo){
            ...
            onMounted(fun(){
                if (props.text != "") {
                    itemId.value = props.text.toString().charCodeAt(0).toString(10)
                }
                init()
            }
            )
            val setId = fun(idPam: String) {
                console.log(idPam, " at uni_modules/uview-ultra/components/up-index-item/up-index-item.uvue:73")
                itemId.value = idPam
            }
            val getRefs = fun(): UTSJSONObject {
                return _uO("height" to height.value)
            }
            __expose(_uM("setId" to setId, "getRefs" to getRefs))
            return fun(): Any? {
                return _cE("view", _uM("class" to _nC(_uA(
                    "up-index-item",
                    _uA(
                        "up-index-item-" + itemId.value
                    )
                )), "id" to ("up-index-item-" + itemId.value)), _uA(
