...
## 1.3.9 用 Markdown 渲染时，mp-html.uvue 相对上游的 10 处改造 + 表格 / 图片 / 代码块 / 视频 / 文字颜色的坑

渲染链路是 `kux-marked`（UTS 版 marked）把 markdown 解析成 HTML，再交给项目内已适配 uni-app X 的 `mp-html.uvue` 原生渲染。**本项目保留的是轻量版 `mp-html.uvue`（未接线同目录那份闲置的 `parser.uts` / `node.uvue`）**，并就地做了 10 处改造（文件头有同样的清单）：
